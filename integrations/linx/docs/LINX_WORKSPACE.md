...
UART:
- The default constraints map `uart_tx` to **PMOD JE pin 1** (`platforms/zybo_z7_20/constraints/zybo_z7_20_linx_cpu.xdc`).
- Connect an external USB-UART adapter RX to JE pin 1 and open a terminal at **115200 8N1**.

Program image:
- Default memory image is `platforms/zybo_z7_20/programs/test_or.memh`.
- Replace it with a UART hello program by generating a new `.memh` and updating `defparam ... INIT_MEMH` in `platforms/zybo_z7_20/rtl/zybo_linx_cpu_top.sv`.
...
```bash
bash integrations/linx/flows/tools/wsl/bootstrap_pyc_ubuntu2204.sh
```

In WSL, install deps + print the suggested build commands for LLVM (LinxISA backend), QEMU, docs, and regression:

```bash
bash integrations/linx/flows/tools/wsl/bootstrap_linx_ubuntu2204.sh
```

Full WSL usage + build script:

- `docs/WSL_UBUNTU_ON_WINDOWS.md`
  (now under `integrations/linx/docs/WSL_UBUNTU_ON_WINDOWS.md`)
...
bash integrations/linx/designs/linxcore/tools/image/build_linxisa_benchmarks_memh_compat.sh
```
