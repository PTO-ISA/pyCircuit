...
def _parse_symtab(
    data: bytes, sections: list[Section], symtab: Section
) -> list[Symbol]:
    ...
    for sec in sections:
        ...
        for off in range(0, sec.sh_size, sec.sh_entsize):
            ...
            if sym_index < 0 or sym_index >= len(symtab):
                raise ValueError(
                    f"reloc @{r_offset:#x}: sym index out of range: {sym_index}"
                )
            ...
            if sym.st_shndx not in section_addrs:
                raise ValueError(
                    f"reloc @{r_offset:#x}: no address for section {sym.st_shndx}"
                )
            ...
            if r_offset < 0 or r_offset >= len(text):
                raise ValueError(
                    f"reloc @{r_offset:#x}: out of bounds for .text size {len(text)}"
                )

            def read_u16(off: int) -> int:
                if off + 2 > len(text):
                    raise ValueError(
                        f"reloc @{r_offset:#x}: out of bounds u16 read at {off:#x} (.text size {len(text)})"
                    )
            ...
            def write_u16(off: int, v: int) -> None:
                if off + 2 > len(text):
                    raise ValueError(
                        f"reloc @{r_offset:#x}: out of bounds u16 write at {off:#x} (.text size {len(text)})"
                    )
                text[off : off + 2] = int(v & 0xFFFF).to_bytes(
            ...
            def read_u32(off: int) -> int:
                if off + 4 > len(text):
                    raise ValueError(
                        f"reloc @{r_offset:#x}: out of bounds u32 read at {off:#x} (.text size {len(text)})"
                    )
            ...
            def write_u32(off: int, v: int) -> None:
                if off + 4 > len(text):
                    raise ValueError(
                        f"reloc @{r_offset:#x}: out of bounds u32 write at {off:#x} (.text size {len(text)})"
                    )
                text[off : off + 4] = int(v & 0xFFFF_FFFF).to_bytes(
            ...
            if r_type == 15:
                ...
                if (insn & 0x7F) != 0x07:
                    raise ValueError(
                        f"hi20 reloc @{r_offset:#x}: expected ADDTPC, got insn=0x{insn:08x}"
                    )
                p_page = P & ~0xFFF
            ...
            if r_type == 17:
                ...
                if (insn & 0x707F) != 0x0015:
                    raise ValueError(
                        f"lo12 reloc @{r_offset:#x}: expected ADDI, got insn=0x{insn:08x}"
                    )
                p_page = P & ~0xFFF
                delta = int((S + addend) - p_page)
                uimm12 = delta & 0xFFF
            ...
            if r_type == 5:
                ...
                if not masked_eq(insn48, mask=0x00007FFF000F, match=0x00004001000E):
                    raise ValueError(
                        f"hl_bstart_call reloc @{r_offset:#x}: unexpected insn48=0x{insn48:012x}"
                    )
            ...
            if r_type == 20:
                ...
                if not masked_eq(insn48, mask=0x0000007F000F, match=0x00000039000E):
                    raise ValueError(
                        f"hl_lw_pcr reloc @{r_offset:#x}: unexpected insn48=0x{insn48:012x}"
                    )
                delta = int((S + addend) - P)
                simm = delta
                hi12 = (simm >> 17) & 0xFFF
            ...
            if r_type == 21:
                ...
                if not masked_eq(insn48, mask=0x0000007F000F, match=0x00000069000E):
                    raise ValueError(
                        f"hl_sw_pcr reloc @{r_offset:#x}: unexpected insn48=0x{insn48:012x}"
                    )
                delta = int((S + addend) - P)
                simm = delta
                hi12 = (simm >> 17) & 0xFFF
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Build a memory init (.memh) from an ELF (relocatable or executable)."
    )
    ...
    ap.add_argument(
        "--text-base",
        default="0x10000",
        help="Base address for .text when input is ET_REL (hex)",
    )
    ap.add_argument(
        "--data-base",
        default="0x20000",
        help="Base address for .data when input is ET_REL (hex)",
    )
    ap.add_argument(
        "--page-align",
        default="0x1000",
        help="Alignment for section placement when ET_REL (hex)",
    )
    ap.add_argument(
        "--start-symbol",
        default="_start",
        help="Symbol to use as boot PC when emitting metadata (default: _start)",
    )
    ap.add_argument(
        "--print-start",
        action="store_true",
        help="Print resolved start PC (hex) to stdout",
    )
    ap.add_argument(
        "--print-max",
        action="store_true",
    ...
    text_bytes = bytearray(
        data[text_sec.sh_offset : text_sec.sh_offset + text_sec.sh_size]
    )
    ...
    if e_type == ET_REL:
        ...
        raise SystemExit(
            f"error: unsupported ELF type {e_type} (expected ET_REL={ET_REL} or ET_EXEC={ET_EXEC})"
        )
    ...
    for sec in sections:
        ...
        if sec.sh_type == SHT_NOBITS:
            ...
            segments.append(
                (
                    section_addrs[sec.index],
                    data[sec.sh_offset : sec.sh_offset + sec.sh_size],
                )
            )
    ...
    if ns.print_start:
        ...
        for sym in symtab:
            ...
            if e_type == ET_REL:
                if sym.st_shndx not in section_addrs:
                    raise SystemExit(
                        f"error: start symbol {start_sym!r} has unknown section index {sym.st_shndx}"
                    )
