...
class Monitor:
    def __init__(self, port: str, baud: int, timeout_s: float) -> None:
        if serial is None:
            _die(
                f"missing pyserial ({_serial_import_error}); install with: pip install -r flows/tools/zybo/requirements.txt"
            )
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Run Linx FPGA regression/benchmarks via the Zybo PS monitor UART protocol."
    )
    ap.add_argument(
        "--port", required=True, help="Windows COM port (e.g. COM5) or /dev/ttyUSB0"
    )
    ap.add_argument(
        "--baud", type=int, default=115200, help="PS UART baud rate (default: 115200)"
    )
    ap.add_argument("--manifest", required=True, type=Path)
    ap.add_argument(
        "--timeout", type=float, default=60.0, help="Per-test timeout in seconds"
    )
    ap.add_argument(
        "--monitor-timeout",
        type=float,
        default=0.5,
        help="UART read timeout in seconds",
    )
    args = ap.parse_args()
    ...
    try:
        ...
        for tc in tests:
            ...
            if exit_code != tc.expected_exit:
                print(
                    f"[fpga] FAIL {tc.name}: expected_exit=0x{tc.expected_exit:08x}",
                    file=sys.stderr,
                )
                failures += 1
