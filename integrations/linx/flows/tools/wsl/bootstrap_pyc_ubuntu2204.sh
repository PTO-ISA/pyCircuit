...
# This script lives under integrations/linx/flows/tools/wsl; resolve the
# pyCircuit repository root from that fixed layout.
...
log "emit+compile examples/pycircuit/counter/counter.py (verilog)"
PYTHONDONTWRITEBYTECODE=1 PYTHONPATH="${ROOT_DIR}/python/pycircuit/src" \
  python3 -m pycircuit.cli emit "${ROOT_DIR}/examples/pycircuit/counter/counter.py" -o /tmp/counter.pyc
