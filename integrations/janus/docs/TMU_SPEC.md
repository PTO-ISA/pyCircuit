...
> 版本: 1.0
> 日期: 2026-02-10
> 实现代码: `integrations/janus/pyc/janus/tmu/janus_tmu_pyc.py`
...
配套工具：
- `integrations/janus/tools/plot_tmu_trace.py`: 将 trace CSV 渲染为 SVG 时序图
- `integrations/janus/tools/animate_tmu_trace.py`: 生成 Ring 拓扑动画 SVG
- `integrations/janus/tools/animate_tmu_ring_vcd.py`: 从 VCD 波形生成 Ring 动画
...
| 文件 | 用途 |
|------|------|
| `integrations/janus/pyc/janus/tmu/janus_tmu_pyc.py` | TMU RTL 实现（pyCircuit DSL） |
| `integrations/janus/tb/tb_janus_tmu_pyc.cpp` | C++ cycle-accurate 测试平台 |
| `integrations/janus/tb/tb_janus_tmu_pyc.sv` | SystemVerilog 测试平台 |
| `integrations/janus/tools/run_janus_tmu_pyc_cpp.sh` | C++ 仿真运行脚本 |
| `integrations/janus/tools/run_janus_tmu_pyc_verilator.sh` | Verilator 仿真运行脚本 |
| `integrations/janus/tools/update_tmu_generated.sh` | 重新生成 RTL 脚本 |
| `.pycircuit_out/integrations/janus/janus_tmu_pyc/` | 生成的 Verilog 和 C++ header |
