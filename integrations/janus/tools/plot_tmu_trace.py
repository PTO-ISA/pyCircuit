...
def render_svg(events, max_cycle, max_node, scale, lane_h, out_path: Path):
    ...
    for n in range(max_node + 1):
        ...
        lines.append(
            f"<line class='mid' x1='{margin_x}' y1='{mid_y}' x2='{width - margin_x}' y2='{mid_y}' />"
        )
    ...
    for cycle, node, event, tag, write in events:
        ...
        if is_accept:
            lines.append(
                f"<circle cx='{x}' cy='{y}' r='3.5' fill='{color}' stroke='#1e3a8a' stroke-width='0.5'>"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Render TMU trace CSV into SVG timeline."
    )
    parser.add_argument("csv", type=Path, help="Path to tmu_trace.csv")
    parser.add_argument(
        "-o", "--out", type=Path, default=Path("tmu_trace.svg"), help="Output SVG path"
    )
