...
def parse_vcd(path: Path, watch_names, max_cycles=None, skip_cycles=0):
    ...
    id_to_name = {}
    values = dict.fromkeys(watch_names, "0")
    snapshots = []
...
def emit_token(
    lines, token_id, start_xy, end_xy, begin_s, dur_s, color, shape, label, glow_id
):
    x0, y0 = start_xy
    x1, y1 = end_xy
    if shape == "circle":
...
def render_svg(snapshots, out_path: Path, cycle_time):
    ...
    lines.append("<text class='title' x='30' y='36'>TMU ring flow (from VCD)</text>")
    lines.append(
        f"<text class='legend' x='30' y='58'>req cw/cc = blue/cyan • rsp cw/cc = green/lime • {cycle_time:.2f}s per cycle</text>"
    )

    lines.append(
        f"<circle class='ring' cx='{cx:.2f}' cy='{cy:.2f}' r='{req_radius:.2f}' />"
    )
    lines.append(
        f"<circle class='ring-rsp' cx='{cx:.2f}' cy='{cy:.2f}' r='{rsp_radius:.2f}' />"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Animate TMU ring flows from VCD debug signals."
    )
    parser.add_argument("vcd", type=Path, help="Path to VCD (tb_janus_tmu_pyc_cpp.vcd)")
    parser.add_argument(
        "-o", "--out", type=Path, default=Path("tmu_flow_real.svg"), help="Output SVG"
    )
    ...
    parser.add_argument(
        "--skip-cycles", type=int, default=0, help="Skip initial cycles"
    )
    ...
    snapshots = parse_vcd(
        args.vcd, watch, max_cycles=args.max_cycles, skip_cycles=args.skip_cycles
    )
    if not snapshots:
