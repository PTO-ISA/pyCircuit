...
def emit_token(lines, token_id, coords, begin_s, dur_s, color, shape, label):
    ...
    if shape == "circle":
        lines.append(
            f"<circle id='{token_id}' cx='{coords[0][0]:.2f}' cy='{coords[0][1]:.2f}' r='6' fill='{color}' stroke='#111827' stroke-width='1'>"
        )
    else:
        size = 7
        x0, y0 = coords[0]
        ...
        lines.append(
            f"<polygon id='{token_id}' points='{' '.join(points)}' fill='{color}' stroke='#111827' stroke-width='1'>"
        )
...
def render_svg(transactions, max_cycle, out_path: Path, cycle_time):
    ...
    lines.append(
        f"<circle class='ring' cx='{cx:.2f}' cy='{cy:.2f}' r='{radius:.2f}' />"
    )
    lines.append("<text class='title' x='30' y='36'>TMU ring flow animation</text>")
    lines.append(
        "<text class='legend' x='30' y='56'>blue=accept(req), green=resp</text>"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Render animated SVG for TMU ring flows."
    )
    parser.add_argument("csv", type=Path, help="Path to tmu_trace.csv")
    parser.add_argument(
        "-o", "--out", type=Path, default=Path("tmu_flow.svg"), help="Output SVG"
    )
