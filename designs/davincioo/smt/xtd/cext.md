# SMT.XTD.CEXT — Consumer Extension

- Source candidate: `DAV-SMT-XTD-CEXT-0001`
- Hardware hierarchy: **H3**, within H1 `SMT` / H2 `XTD`
- NDF refinement: **L2 microarchitecture**; module-specific L1 behavior links still require review.
- Recommended disposition: **review** (proposal, not registry approval)
- Proposed implementation path, if accepted as an independent leaf: `designs/davincioo/smt/xtd/cext.py`
- Current design-program execution status: **not implemented**. External source evidence is recorded separately.

deferred interface. Consumer-extension relation needs explicit operand lifetime

## Inputs

| Name | Payload/type | Meaning | Evidence status |
| --- | --- | --- | --- |
| extension_event | TBD | extension event | unresolved |

## Outputs

| Name | Payload/type | Meaning | Evidence status |
| --- | --- | --- | --- |
| extension_result | TBD | extension result | unresolved |

Payload names in proposed rows are design pseudotypes until fields, widths and nominal identity are frozen. Queue transport is inferred by the compiler, not a requested public Queue wrapper. Clock/reset/time domain are execution context and must not be invented as ordinary payload ports.

## Owned or containing state

- State ownership unresolved: Consumer-extension relation needs explicit operand lifetime
...
- Resolve the owner/disposition and exact typed seam before adding a source or registration entry.

gfsim execution is the first implementation gate. PYC/RTL obligations apply to the admitted lowering and remain explicit future work where provisional storage is rejected. Compile-only evidence does not establish behavior.

## Open decisions

- No architecture or lifetime contract currently freezes this extension; identify producer, consumer and reclaim authority before implementation.
...
- `docs/specification/davincioo/ndf-next/interfaces/pycircuit-module-catalog.json:5637` — Generated catalog identity, hierarchy and planned source path.
- `docs/architecture/core/l3/FRONTEND_CONTEXT_PACKETS.md:81` — Design review disposition: deferred interface; Consumer-extension relation needs explicit operand lifetime
