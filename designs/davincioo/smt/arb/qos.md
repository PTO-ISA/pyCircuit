# SMT.ARB.QOS — Quality of Service

- Source candidate: `DAV-SMT-ARB-QOS-0001`
...
- Proposed implementation path, if accepted as an independent leaf: `designs/davincioo/smt/arb/qos.py`
- Current design-program execution status: **not implemented**. External source evidence is recorded separately.

deferred policy. QoS parameters cannot silently replace fair progress

## Inputs

| Name | Payload/type | Meaning | Evidence status |
| --- | --- | --- | --- |
| candidate | TBD | candidate | unresolved |
| ack | TBD | ack | unresolved |
| cancel | EpochCancel | cancel | proposed |

## Outputs

| Name | Payload/type | Meaning | Evidence status |
| --- | --- | --- | --- |
| grant | TBD | grant | unresolved |

Payload names in proposed rows are design pseudotypes until fields, widths and nominal identity are frozen. Queue transport is inferred by the compiler, not a requested public Queue wrapper. Clock/reset/time domain are execution context and must not be invented as ordinary payload ports.

## Owned or containing state

- State ownership unresolved: QoS parameters cannot silently replace fair progress
...
- `docs/specification/davincioo/ndf-next/interfaces/pycircuit-module-catalog.json:4936` — Generated catalog identity, hierarchy and planned source path.
- `docs/architecture/core/l3/FRONTEND_CONTEXT_PACKETS.md:68` — Design review disposition: deferred policy; QoS parameters cannot silently replace fair progress
- `docs/specification/davincioo/ndf-next/scalar/smt.md:88` — Normative candidate clause (active status).
