# SMT.THR.TPC — Thread Program Counter

- Source candidate: `DAV-SMT-THR-TPC-0001`
- Hardware hierarchy: **H3**, within H1 `SMT` / H2 `THR`
- NDF refinement: **L2 microarchitecture**; module-specific L1 behavior links still require review.
- Recommended disposition: **leaf** (proposal, not registry approval)
- Proposed implementation path, if accepted as an independent leaf: `designs/davincioo/smt/thr/tpc.py`
- Current design-program execution status: **not implemented**. External source evidence is recorded separately.

leaf; detailed. Per-flow speculative fetch PC

## Inputs

| Name | Payload/type | Meaning | Evidence status |
| --- | --- | --- | --- |
| pc_read | PcRequest | pc read | proposed |
| sequential_advance | PcAdvance | sequential advance | proposed |
| redirect | PcRedirect | redirect | proposed |
| start | PcStart | start | proposed |

## Outputs

| Name | Payload/type | Meaning | Evidence status |
| --- | --- | --- | --- |
| pc_reply | PcReply | pc reply | proposed |
| pc_update_ack | PcUpdateAck | pc update ack | proposed |

Payload names in proposed rows are design pseudotypes until fields, widths and nominal identity are frozen. Queue transport is inferred by the compiler, not a requested public Queue wrapper. Clock/reset/time domain are execution context and must not be invented as ordinary payload ports.

## Owned or containing state

- PcTable[FlowKey]: speculative fetch PC, epoch, version, outstanding advance lease and start-valid
...
- `docs/specification/davincioo/ndf-next/interfaces/pycircuit-module-catalog.json:5551` — Generated catalog identity, hierarchy and planned source path.
- `docs/architecture/core/l3/FRONTEND_CONTEXT_PACKETS.md:59` — Design review disposition: leaf; detailed; Per-flow speculative fetch PC
- `docs/architecture/core/l3/FRONTEND_CONTEXT_PACKETS.md:146` — Detailed proposed state and Queue contract.
- `docs/specification/davincioo/ndf-next/scalar/smt.md:35` — Normative candidate clause (active status).
