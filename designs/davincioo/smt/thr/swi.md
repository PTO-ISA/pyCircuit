# SMT.THR.SWI — Thread Switch

- Source candidate: `DAV-SMT-THR-SWI-0001`
...
- Proposed implementation path, if accepted as an independent leaf: `designs/davincioo/smt/thr/swi.py`
- Current design-program execution status: **not implemented**. External source evidence is recorded separately.

interface. CTX runnable/start/stop command, not a second context table

## Inputs

| Name | Payload/type | Meaning | Evidence status |
| --- | --- | --- | --- |
| request | TBD | request | unresolved |

## Outputs

| Name | Payload/type | Meaning | Evidence status |
| --- | --- | --- | --- |
| response | TBD | response | unresolved |

Payload names in proposed rows are design pseudotypes until fields, widths and nominal identity are frozen. Queue transport is inferred by the compiler, not a requested public Queue wrapper. Clock/reset/time domain are execution context and must not be invented as ordinary payload ports.

## Owned or containing state

- State ownership unresolved: CTX runnable/start/stop command, not a second context table
...
- `docs/specification/davincioo/ndf-next/interfaces/pycircuit-module-catalog.json:5465` — Generated catalog identity, hierarchy and planned source path.
- `docs/architecture/core/l3/FRONTEND_CONTEXT_PACKETS.md:63` — Design review disposition: interface; CTX runnable/start/stop command, not a second context table
- `docs/specification/davincioo/ndf-next/scalar/smt.md:54` — Normative candidate clause (active status).
