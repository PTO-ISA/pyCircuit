...
- Source candidate: `DAV-VEC-SHU-GTH-0001`
...
mode in PERM initially. Promote only if indexed gather needs independent memory-style queues.
...
- No independent owner; realize within or at the reviewed parent boundary: Promote only if indexed gather needs independent memory-style queues.
...
- `docs/specification/davincioo/ndf-next/interfaces/pycircuit-module-catalog.json:6911` — Generated catalog identity, hierarchy and planned source path.
- `docs/architecture/core/l3/COMPUTE_GROUP_PACKETS.md:142` — Design review disposition: mode in PERM initially; Promote only if indexed gather needs independent memory-style queues.
