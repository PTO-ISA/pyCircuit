...
- Source candidate: `DAV-VEC-SHU-SCT-0001`
...
mode in PERM initially. Promote only if collision policy requires a separate owner.
...
- No independent owner; realize within or at the reviewed parent boundary: Promote only if collision policy requires a separate owner.
...
- `docs/specification/davincioo/ndf-next/interfaces/pycircuit-module-catalog.json:6991` — Generated catalog identity, hierarchy and planned source path.
- `docs/architecture/core/l3/COMPUTE_GROUP_PACKETS.md:143` — Design review disposition: mode in PERM initially; Promote only if collision policy requires a separate owner.
