# VEC.VEX.DBUF — Destination Buffer

- Source candidate: `DAV-VEC-VEX-DBUF-0001`
- Hardware hierarchy: **H3**, within H1 `VEC` / H2 `VEX`
- NDF refinement: **L2 microarchitecture**; module-specific L1 behavior links still require review.
- Recommended disposition: **leaf** (proposal, not registry approval)
- Proposed implementation path, if accepted as an independent leaf: `designs/davincioo/vec/vex/dbuf.py`
- Current design-program execution status: **not implemented**. External source evidence is recorded separately.

leaf. Owns shadow-result beat table and drain cursor.

## Inputs

| Name | Payload/type | Meaning | Evidence status |
| --- | --- | --- | --- |
| fragment_in | ResultFragment | fragment in | proposed |
...
| vec_resolve_out | EngineResolve | vec resolve out | proposed |

Payload names in proposed rows are design pseudotypes until fields, widths and nominal identity are frozen. Queue transport is inferred by the compiler, not a requested public Queue wrapper. Clock/reset/time domain are execution context and must not be invented as ordinary payload ports.

## Owned or containing state

- Dest[depth] with OpKey, destination, received/accepted bitmaps, defined region, fault and resolve-sent
...
- `docs/specification/davincioo/ndf-next/interfaces/pycircuit-module-catalog.json:7373` — Generated catalog identity, hierarchy and planned source path.
- `docs/architecture/core/l3/COMPUTE_GROUP_PACKETS.md:129` — Design review disposition: leaf; Owns shadow-result beat table and drain cursor.
- `docs/architecture/core/l3/COMPUTE_GROUP_PACKETS.md:306` — Detailed proposed state and Queue contract.
