# VEC.VEX.ALU — Arithmetic Logic Unit

- Source candidate: `DAV-VEC-VEX-ALU-0001`
- Hardware hierarchy: **H3**, within H1 `VEC` / H2 `VEX`
- NDF refinement: **L2 microarchitecture**; module-specific L1 behavior links still require review.
- Recommended disposition: **leaf** (proposal, not registry approval)
- Proposed implementation path, if accepted as an independent leaf: `designs/davincioo/vec/vex/alu.py`
- Current design-program execution status: **not implemented**. External source evidence is recorded separately.

leaf (`VECALU`). Owns elementwise pipeline occupancy, not operand storage.
...
- Pipe`[latency][lanes]` carrying OpKey, lane-valid mask, numeric control, fragments, result and fault bits
...
- `docs/specification/davincioo/ndf-next/interfaces/pycircuit-module-catalog.json:7330` — Generated catalog identity, hierarchy and planned source path.
- `docs/architecture/core/l3/COMPUTE_GROUP_PACKETS.md:130` — Design review disposition: leaf (`VECALU`); Owns elementwise pipeline occupancy, not operand storage.
- `docs/architecture/core/l3/COMPUTE_GROUP_PACKETS.md:248` — Detailed proposed state and Queue contract.
- `docs/specification/davincioo/ndf-next/tile/vec.md:15` — Normative candidate clause (draft status).
