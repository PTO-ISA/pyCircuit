# VEC.VEX.ACC — Accumulation

- Source candidate: `DAV-VEC-VEX-ACC-0001`
...
state in RED initially. Separate leaf only if concurrent segmented reductions require it.
...
- No independent owner; realize within or at the reviewed parent boundary: Separate leaf only if concurrent segmented reductions require it.
...
- `docs/specification/davincioo/ndf-next/interfaces/pycircuit-module-catalog.json:7293` — Generated catalog identity, hierarchy and planned source path.
- `docs/architecture/core/l3/COMPUTE_GROUP_PACKETS.md:132` — Design review disposition: state in RED initially; Separate leaf only if concurrent segmented reductions require it.
