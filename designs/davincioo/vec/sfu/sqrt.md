# VEC.SFU.SQRT — Square Root

- Source candidate: `DAV-VEC-SFU-SQRT-0001`
- Hardware hierarchy: **H3**, within H1 `VEC` / H2 `SFU`
- NDF refinement: **L2 microarchitecture**; module-specific L1 behavior links still require review.
- Recommended disposition: **leaf** (proposal, not registry approval)
- Proposed implementation path, if accepted as an independent leaf: `designs/davincioo/vec/sfu/sqrt.py`
- Current design-program execution status: **not implemented**. External source evidence is recorded separately.

leaf-family instance. Same SFU shell; preserves exception metadata.
...
- Independent bounded state is expected but its exact schema is not frozen: Same SFU shell; preserves exception metadata.
...
- `docs/specification/davincioo/ndf-next/interfaces/pycircuit-module-catalog.json:6874` — Generated catalog identity, hierarchy and planned source path.
- `docs/architecture/core/l3/COMPUTE_GROUP_PACKETS.md:137` — Design review disposition: leaf-family instance; Same SFU shell; preserves exception metadata.
