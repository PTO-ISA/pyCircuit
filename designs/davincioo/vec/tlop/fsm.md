# VEC.TLOP.FSM — Finite State Machine

- Source candidate: `DAV-VEC-TLOP-FSM-0001`
- Hardware hierarchy: **H3**, within H1 `VEC` / H2 `TLOP`
- NDF refinement: **L2 microarchitecture**; module-specific L1 behavior links still require review.
- Recommended disposition: **leaf** (proposal, not registry approval)
- Proposed implementation path, if accepted as an independent leaf: `designs/davincioo/vec/tlop/fsm.py`
- Current design-program execution status: **not implemented**. External source evidence is recorded separately.

leaf. Owns parent/child progress and expansion cursor.

## Inputs

| Name | Payload/type | Meaning | Evidence status |
| --- | --- | --- | --- |
| engine_command_in | EngineCommand | engine command in | proposed |
| child_resolve_in | EngineResolve | child resolve in | proposed |
...
| Name | Payload/type | Meaning | Evidence status |
| --- | --- | --- | --- |
| child_uop_out | EngineCommand | child uop out | proposed |
| engine_resolve_out | EngineResolve | engine resolve out | proposed |

Payload names in proposed rows are design pseudotypes until fields, widths and nominal identity are frozen. Queue transport is inferred by the compiler, not a requested public Queue wrapper. Clock/reset/time domain are execution context and must not be invented as ordinary payload ports.

## Owned or containing state

- Independent bounded state is expected but its exact schema is not frozen: Owns parent/child progress and expansion cursor.
...
- `docs/specification/davincioo/ndf-next/interfaces/pycircuit-module-catalog.json:7176` — Generated catalog identity, hierarchy and planned source path.
- `docs/architecture/core/l3/COMPUTE_GROUP_PACKETS.md:148` — Design review disposition: leaf; Owns parent/child progress and expansion cursor.
- `docs/specification/davincioo/ndf-next/tile/vec.md:53` — Normative candidate clause (draft status).
