...
- Source candidate: `DAV-CUBE-WBK-FMT-0001`
- Hardware hierarchy: **H3**, within H1 `CUBE` / H2 `WBK`
- NDF refinement: **L2 microarchitecture**; module-specific L1 behavior links still require review.
- Recommended disposition: **leaf** (proposal, not registry approval)
- Proposed implementation path, if accepted as an independent leaf: `designs/davincioo/cube/wbk/fmt.py`
- Current design-program execution status: **not implemented**. External source evidence is recorded separately.

leaf or WBF substage. Promote when layout formatting requires multi-cycle storage.

## Inputs

| Name | Payload/type | Meaning | Evidence status |
| --- | --- | --- | --- |
| fragment_or_ack_in | TBD | fragment or ack in | unresolved |
...
| Name | Payload/type | Meaning | Evidence status |
| --- | --- | --- | --- |
| write_or_resolve_out | TBD | write or resolve out | unresolved |

Payload names in proposed rows are design pseudotypes until fields, widths and nominal identity are frozen. Queue transport is inferred by the compiler, not a requested public Queue wrapper. Clock/reset/time domain are execution context and must not be invented as ordinary payload ports.

## Owned or containing state

- Independent bounded state is expected but its exact schema is not frozen: Promote when layout formatting requires multi-cycle storage.
...
- `docs/specification/davincioo/ndf-next/interfaces/pycircuit-module-catalog.json:8143` — Generated catalog identity, hierarchy and planned source path.
- `docs/architecture/core/l3/COMPUTE_GROUP_PACKETS.md:172` — Design review disposition: leaf or WBF substage; Promote when layout formatting requires multi-cycle storage.
