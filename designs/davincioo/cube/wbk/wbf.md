# CUBE.WBK.WBF — Writeback Buffer

- Source candidate: `DAV-CUBE-WBK-WBF-0001`
- Hardware hierarchy: **H3**, within H1 `CUBE` / H2 `WBK`
- NDF refinement: **L2 microarchitecture**; module-specific L1 behavior links still require review.
- Recommended disposition: **leaf** (proposal, not registry approval)
- Proposed implementation path, if accepted as an independent leaf: `designs/davincioo/cube/wbk/wbf.py`
- Current design-program execution status: **not implemented**. External source evidence is recorded separately.

leaf. Owns formatted shadow beats until TMU accepts writes.

## Inputs

| Name | Payload/type | Meaning | Evidence status |
| --- | --- | --- | --- |
| converted_fragment_in | ConvertedFragment | converted fragment in | proposed |
| tile_write_ack_in | CellWriteAck | tile write ack in | proposed |
...
| Name | Payload/type | Meaning | Evidence status |
| --- | --- | --- | --- |
| tile_write_req_out | CellWriteReq | tile write req out | proposed |
| cube_resolve_out | EngineResolve | cube resolve out | proposed |

Payload names in proposed rows are design pseudotypes until fields, widths and nominal identity are frozen. Queue transport is inferred by the compiler, not a requested public Queue wrapper. Clock/reset/time domain are execution context and must not be invented as ordinary payload ports.

## Owned or containing state

- WriteOp[depth] and BeatData with produced/accepted masks, destination generation, defined region, fault and resolve-sent
...
- `docs/specification/davincioo/ndf-next/interfaces/pycircuit-module-catalog.json:8180` — Generated catalog identity, hierarchy and planned source path.
- `docs/architecture/core/l3/COMPUTE_GROUP_PACKETS.md:171` — Design review disposition: leaf; Owns formatted shadow beats until TMU accepts writes.
- `docs/architecture/core/l3/COMPUTE_GROUP_PACKETS.md:422` — Detailed proposed state and Queue contract.
- `docs/specification/davincioo/ndf-next/tile/cube.md:51` — Normative candidate clause (draft status).
