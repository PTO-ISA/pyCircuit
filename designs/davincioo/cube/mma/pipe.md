# CUBE.MMA.PIPE — Pipeline

- Source candidate: `DAV-CUBE-MMA-PIPE-0001`
- Hardware hierarchy: **H3**, within H1 `CUBE` / H2 `MMA`
- NDF refinement: **L2 microarchitecture**; module-specific L1 behavior links still require review.
- Recommended disposition: **assembly** (proposal, not registry approval)
...
assembly boundary. Owns queues between MAC/KACC; does not duplicate their state.

## Inputs

| Name | Payload/type | Meaning | Evidence status |
| --- | --- | --- | --- |
| operand_or_partial_in | TBD | operand or partial in | unresolved |
...
| Name | Payload/type | Meaning | Evidence status |
| --- | --- | --- | --- |
| partial_or_status_out | TBD | partial or status out | unresolved |

Payload names in proposed rows are design pseudotypes until fields, widths and nominal identity are frozen. Queue transport is inferred by the compiler, not a requested public Queue wrapper. Clock/reset/time domain are execution context and must not be invented as ordinary payload ports.

## Owned or containing state

- State ownership unresolved: Owns queues between MAC/KACC; does not duplicate their state.
...
- `docs/specification/davincioo/ndf-next/interfaces/pycircuit-module-catalog.json:8063` — Generated catalog identity, hierarchy and planned source path.
- `docs/architecture/core/l3/COMPUTE_GROUP_PACKETS.md:163` — Design review disposition: assembly boundary; Owns queues between MAC/KACC; does not duplicate their state.
- `docs/specification/davincioo/ndf-next/tile/cube.md:15` — Normative candidate clause (draft status).
