# CUBE.ACC.IACC — Internal Accumulator

- Source candidate: `DAV-CUBE-ACC-IACC-0001`
- Hardware hierarchy: **H3**, within H1 `CUBE` / H2 `ACC`
- NDF refinement: **L2 microarchitecture**; module-specific L1 behavior links still require review.
- Recommended disposition: **leaf** (proposal, not registry approval)
- Proposed implementation path, if accepted as an independent leaf: `designs/davincioo/cube/acc/iacc.py`
- Current design-program execution status: **not implemented**. External source evidence is recorded separately.

leaf. Owns operation-qualified internal accumulator arrays.

## Inputs

| Name | Payload/type | Meaning | Evidence status |
| --- | --- | --- | --- |
| acc_alloc_in | AccAllocate | acc alloc in | proposed |
| ordered_partial_in | MacPartial | ordered partial in | proposed |
| acc_read_req_in | AccReadReq | acc read req in | proposed |
...
| Name | Payload/type | Meaning | Evidence status |
| --- | --- | --- | --- |
| acc_complete_out | AccComplete | acc complete out | proposed |
| acc_read_resp_out | AccReadResp | acc read resp out | proposed |

Payload names in proposed rows are design pseudotypes until fields, widths and nominal identity are frozen. Queue transport is inferred by the compiler, not a requested public Queue wrapper. Clock/reset/time domain are execution context and must not be invented as ordinary payload ports.

## Owned or containing state

- AccMeta[slots] and AccData`[slots][rows][cols]` with OpKey, shape, K count, numeric mode, exception and drain refs
...
- `docs/specification/davincioo/ndf-next/interfaces/pycircuit-module-catalog.json:7601` — Generated catalog identity, hierarchy and planned source path.
- `docs/architecture/core/l3/COMPUTE_GROUP_PACKETS.md:167` — Design review disposition: leaf; Owns operation-qualified internal accumulator arrays.
- `docs/architecture/core/l3/COMPUTE_GROUP_PACKETS.md:394` — Detailed proposed state and Queue contract.
- `docs/specification/davincioo/ndf-next/tile/cube.md:43` — Normative candidate clause (draft status).
