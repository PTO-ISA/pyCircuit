# CUBE.DFL.CSCL — Cube Scale

- Source candidate: `DAV-CUBE-DFL-CSCL-0001`
- Hardware hierarchy: **H3**, within H1 `CUBE` / H2 `DFL`
- NDF refinement: **L2 microarchitecture**; module-specific L1 behavior links still require review.
- Recommended disposition: **leaf** (proposal, not registry approval)
- Proposed implementation path, if accepted as an independent leaf: `designs/davincioo/cube/dfl/cscl.py`
- Current design-program execution status: **not implemented**. External source evidence is recorded separately.

leaf or RDB submode. Promote if scale fetch/validation has independent backpressure.

## Inputs

| Name | Payload/type | Meaning | Evidence status |
| --- | --- | --- | --- |
| command_or_fragment_in | TBD | command or fragment in | unresolved |
...
| Name | Payload/type | Meaning | Evidence status |
| --- | --- | --- | --- |
| operand_or_status_out | TBD | operand or status out | unresolved |

Payload names in proposed rows are design pseudotypes until fields, widths and nominal identity are frozen. Queue transport is inferred by the compiler, not a requested public Queue wrapper. Clock/reset/time domain are execution context and must not be invented as ordinary payload ports.

## Owned or containing state

- Independent bounded state is expected but its exact schema is not frozen: Promote if scale fetch/validation has independent backpressure.
...
- `docs/specification/davincioo/ndf-next/interfaces/pycircuit-module-catalog.json:7829` — Generated catalog identity, hierarchy and planned source path.
- `docs/architecture/core/l3/COMPUTE_GROUP_PACKETS.md:160` — Design review disposition: leaf or RDB submode; Promote if scale fetch/validation has independent backpressure.
