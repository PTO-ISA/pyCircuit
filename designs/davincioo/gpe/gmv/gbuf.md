# GPE.GMV.GBUF — Group Buffer

- Source candidate: `DAV-GPE-GMV-GBUF-0001`
- Hardware hierarchy: **H3**, within H1 `GPE` / H2 `GMV`
- NDF refinement: **L2 microarchitecture**; module-specific L1 behavior links still require review.
- Recommended disposition: **leaf** (proposal, not registry approval)
- Proposed implementation path, if accepted as an independent leaf: `designs/davincioo/gpe/gmv/gbuf.py`
- Current design-program execution status: **not implemented**. External source evidence is recorded separately.

leaf. Owns per-group payload staging and remaining-consumer state.

## Inputs

| Name | Payload/type | Meaning | Evidence status |
| --- | --- | --- | --- |
| reserve_in | GroupMoveReserve | reserve in | proposed |
| producer_data_in | CellBeat | producer data in | proposed |
| fabric_rsp_in | GPEPacket | fabric rsp in | proposed |
...
| Name | Payload/type | Meaning | Evidence status |
| --- | --- | --- | --- |
| fabric_req_out | GPEPacket | fabric req out | proposed |
| consumer_data_out | CellBeat[pe] | consumer data out | proposed |
| gpe_resolve_out | EngineResolve | gpe resolve out | proposed |
...
- Buffer[entries] with group/producer identity, TileRef, payload, consumer masks, generation, lease and tombstone
...
- `docs/specification/davincioo/ndf-next/interfaces/pycircuit-module-catalog.json:9595` — Generated catalog identity, hierarchy and planned source path.
- `docs/architecture/core/l3/COMPUTE_GROUP_PACKETS.md:185` — Design review disposition: leaf; Owns per-group payload staging and remaining-consumer state.
- `docs/architecture/core/l3/COMPUTE_GROUP_PACKETS.md:486` — Detailed proposed state and Queue contract.
- `docs/specification/davincioo/ndf-next/tile/gpe.md:46` — Normative candidate clause (draft status).
