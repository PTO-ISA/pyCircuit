# SPE.IEX.S2 — Schedule Stage 2

- Source candidate: `DAV-SPE-IEX-S2-0001`
...
Schedule Stage 2 is a named pipeline phase or contained state, not an independent architectural owner.

## Inputs

| Name | Payload/type | Meaning | Evidence status |
| --- | --- | --- | --- |
| publication | RenamePublication | S1-prepared uop | proposed |
| isq_ack | IssueAdmissionAck | physical row result | proposed |

## Outputs

| Name | Payload/type | Meaning | Evidence status |
| --- | --- | --- | --- |
| isq_request | IssueEntry | retained issue entry | proposed |

Payload names in proposed rows are design pseudotypes until fields, widths and nominal identity are frozen. Queue transport is inferred by the compiler, not a requested public Queue wrapper. Clock/reset/time domain are execution context and must not be invented as ordinary payload ports.

## Owned or containing state

- State is contained in the owning IEX leaf/assembly; do not create a second owner for S2.
...
- `docs/specification/davincioo/ndf-next/interfaces/pycircuit-module-catalog.json:1` — Catalog row: IEX.S2, source srcs/core/spe/iex/s2.py, status planned/deferred.
- `docs/specification/davincioo/ndf-next/scalar/iex.md:15` — Normative NDF owner/refinement clause.
