...
def i2(
    ...
    @ac.rule
    def capture_operands(response):
        nonlocal active, pending, lhs, rhs, lhs_source, rhs_source, src0_resolved, src1_resolved, cancel_tombstones
        ...
        if entry.src0.valid:
            if entry.src0.speculative:
                expected_forward_mask = expected_forward_mask | 1
            else:
                expected_rf_read_mask = expected_rf_read_mask | 1
        if entry.src1.valid:
            if entry.src1.speculative:
                expected_forward_mask = expected_forward_mask | 2
            else:
                expected_rf_read_mask = expected_rf_read_mask | 2
        tombstone = ac.find(
            cancel_tombstones, where=lambda row: row.valid & (row.key == key)
        )
        ...
        if accepted:
            ...
            src0_resolved = request.forward_mask & 1 == 0
            src1_resolved = request.forward_mask & 2 == 0
    ...
    @ac.rule
    def accept_load_dependency(request):
        nonlocal active, pending, lhs, rhs, src0_resolved, src1_resolved, cancel_pending, cancel_payload
    ...
    @ac.rule
    def publish_execute():
        nonlocal active, pending, lhs, rhs, lhs_source, rhs_source, src0_resolved, src1_resolved, execute_outstanding, release_pending, cancel_pending
        if (
            active
            & src0_resolved
            & src1_resolved
    ...
    @ac.rule
    def accept_sink_decision(request):
        nonlocal active, pending, execute_outstanding, release_pending
        key_matches = (
            active
            & execute_outstanding
            & request.valid
    ...
    @ac.rule
    def publish_release():
        nonlocal active, pending, release_pending
        if active & release_pending:
            released = IssueRelease(
                key=pending.key, dispatch=pending.entry.identity.dispatch, valid=True
            )
            active = False
    ...
    def publish_generated_cancel():
        nonlocal active, pending, execute_outstanding, release_pending, cancel_pending, cancel_payload, cancel_tombstones
        ...
        if active & cancel_pending & (tombstone.valid | free.valid):
            ...
            tombstone_value = AttemptTombstone(key=cancel_payload.key, valid=True)
            if tombstone.valid:
    ...
    @ac.rule
    def accept_external_cancel(request):
        nonlocal active, pending, execute_outstanding, release_pending, cancel_pending, cancel_tombstones
        active_key_matches = active & (pending.key == request.key)
        tombstone = ac.find(
            cancel_tombstones, where=lambda row: row.valid & (row.key == request.key)
        )
        free = ac.find(cancel_tombstones, where=lambda row: not row.valid)
        post_accept_cancel = active_key_matches & release_pending
        matched = (
            request.valid & (not post_accept_cancel) & (tombstone.valid | free.valid)
        )
        if matched & (not tombstone.valid):
            cancel_tombstones[free.index] = AttemptTombstone(
                key=request.key, valid=True
            )
    ...
    operand_ack = capture_operands(operand_response)
    dependency_ack = accept_load_dependency(load_dependency)
    execute_request = publish_execute()
    sink_decision_ack = accept_sink_decision(sink_decision)
    release = publish_release()
    generated_cancel = publish_generated_cancel()
    cancel_ack = accept_external_cancel(cancel)
    return (
        execute_request,
        operand_ack,
        dependency_ack,
