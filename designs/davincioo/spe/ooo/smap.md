# SPE.OOO.SMAP — Speculative Map

- Source candidate: `DAV-SPE-OOO-SMAP-0001`
...
Speculative Map is a named pipeline phase or contained state, not an independent architectural owner.

## Inputs

| Name | Payload/type | Meaning | Evidence status |
| --- | --- | --- | --- |
| rename_lookup | MapLookupRequest | REN speculative mapping access | proposed |
| rename_update | MapRecord | REN-owned speculative update | proposed |

## Outputs

| Name | Payload/type | Meaning | Evidence status |
| --- | --- | --- | --- |
| map_result | MapLookupResponse | REN-local mapping result | proposed |

Payload names in proposed rows are design pseudotypes until fields, widths and nominal identity are frozen. Queue transport is inferred by the compiler, not a requested public Queue wrapper. Clock/reset/time domain are execution context and must not be invented as ordinary payload ports.

## Owned or containing state

- State is contained in the owning OOO leaf/assembly; do not create a second owner for SMAP.
...
- `docs/specification/davincioo/ndf-next/interfaces/pycircuit-module-catalog.json:1` — Catalog row: OOO.SMAP, source srcs/core/spe/ooo/smap.py, status planned/deferred.
- `docs/specification/davincioo/ndf-next/scalar/ooo.md:130` — Normative NDF owner/refinement clause.
