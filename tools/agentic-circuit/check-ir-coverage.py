...
- docs/development/acir/verification/ir-coverage.md is the up-to-date generated ledger.
...
OP_DEF_PATTERN = re.compile(r"def\s+(\w+Op)\s*:\s*\w+<\"([^\"]+)\"", re.MULTILINE)
...
def lit_test_files(root):
    test_dir = root / "tests/mlir/agentic-circuit"
    positive = []
    negative = []
    if test_dir.is_dir():
    ...
    for dialect, spec in DIALECTS.items():
        ...
        for full_name in data["manifest"]["operations"]:
            ...
            if errors is not None:
                if not pos:
                    errors.append(f"{full_name} has no positive lit test coverage")
                if not neg:
                    errors.append(f"{full_name} has no negative lit test coverage")
            entries.append(
                {
                    "kind": "operation",
                    "name": full_name,
...
def _ledger_row(entry):
    ...
    return f"| {entry['name']} | {entry['symbol']} | {positive} | {negative} |"
...
def check_ledger(root, surface, errors):
    ...
    if not path.is_file():
        errors.append(
            ...
            f"(run tools/agentic-circuit/check-ir-coverage.py --write-ledger)"
        )
        return
    ...
    if path.read_text() != expected:
        errors.append(
            ...
            f"tools/agentic-circuit/check-ir-coverage.py --write-ledger"
        )
...
def main(argv=None):
    ...
    if errors:
        ...
        return 1
    print(
        "IR coverage: OK (ACIR manifest matches ODS, "
