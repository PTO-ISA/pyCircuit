...
def render(rows: list[dict[str, object]]) -> str:
    ...
    lines = [
        ...
        "",
        "<!-- Generated by tools/pycircuit/check-pyc-inventory.py --write-ledger. Do not edit. -->",
        "",
...
def run_checks(root: Path = ROOT) -> list[str]:
    ...
    if not LEDGER.is_file() or _read(LEDGER) != render(rows):
        errors.append(
            "PYC inventory ledger is stale; run "
            "tools/pycircuit/check-pyc-inventory.py --write-ledger"
        )
    if not COVERAGE_JSON.is_file() or _read(COVERAGE_JSON) != render_json(
        rows, type_rows
    ):
        errors.append(
            "PYC machine-readable coverage is stale; run "
            "tools/pycircuit/check-pyc-inventory.py --write-ledger"
        )
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not LEDGER.is_file() or _read(LEDGER) != render(rows):
        print(
            "error: PYC inventory ledger is stale; run "
            "tools/pycircuit/check-pyc-inventory.py --write-ledger",
            file=sys.stderr,
        )
        return 1
    if not COVERAGE_JSON.is_file() or _read(COVERAGE_JSON) != render_json(
        ...
        print(
            "error: PYC machine-readable coverage is stale; run "
            "tools/pycircuit/check-pyc-inventory.py --write-ledger",
            file=sys.stderr,
        )
        return 1
