...
def render(trace: Path, output: Path) -> Path:
    ...
    document = {
        "objects": replay.manifest["objects"], "initial": replay.initial,
        "commits": replay.commits, "events": events,
        "complete": replay.complete, "diagnostic": replay.diagnostic,
    }
...
def main() -> int:
    ...
    command = commands.add_parser("render", help="Generate a self-contained offline HTML")
    command.add_argument("trace", type=Path)
    command.add_argument("--output", required=True, type=Path)
    args = parser.parse_args()
    try:
        print(render(args.trace, args.output))
        return 0
