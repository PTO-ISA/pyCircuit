  ...
  for (auto port : import.getSchema()
    ...
    if (!concrete || isa<acir::ac::QueueType>(concrete.getType().getValue()))
      return import.emitError(
          "split C++ emission cannot wire an imported queue carrier");
...
static LogicalResult emitBlockStruct(
    ...
    for (auto inst : instances) {
      ...
      if (auto callee = mod.lookupSymbol<func::FuncOp>(calleeAttr.getValue())) {
        ...
        if (failed(computeImportPortNames(import, inPorts, outPorts)))
          return failure();
        calleeName = import.getSymName().str();
      } else {
  ...
  for (auto fifo : asyncFifos) {
    unsigned w = bitWidth(fifo.getOutData().getType());
    if (w == 0 || w > 64)
      return fifo.emitError("C++ emitter only supports async_fifo widths 1..64");
    auto depthAttr = fifo->getAttrOfType<IntegerAttr>("depth");
    if (!depthAttr)
      return fifo.emitError("missing integer attribute `depth`");
    auto depth = depthAttr.getValue().getZExtValue();
  ...
  for (auto s : cdcSyncs) {
    ...
    if (w == 0 || w > 64)
      return s.emitError("C++ emitter only supports cdc_sync widths 1..64");
