...
static llvm::cl::opt<std::string>
    emitKind("emit", llvm::cl::desc("Emission target: verilog|cpp|none"),
             llvm::cl::init("verilog"));

static llvm::cl::opt<std::string> directCppOut(
    "cpp",
    llvm::cl::desc(
        "Direct C++ output path (equivalent to --emit=cpp -o <path>)"),
    llvm::cl::init(""));

static llvm::cl::opt<std::string> directVerilogOut(
    "verilog",
    llvm::cl::desc(
        "Direct Verilog output path (equivalent to --emit=verilog -o <path>)"),
    llvm::cl::init(""));

static llvm::cl::opt<std::string>
    cppSplitMode("cpp-split",
                 llvm::cl::desc("C++ out-dir split mode: module|none"),
                 llvm::cl::init("module"));

static llvm::cl::opt<unsigned> cppShardThresholdLines(
    "cpp-shard-threshold-lines",
    llvm::cl::desc("Shard oversized C++ module sources when generated line "
                   "count exceeds this threshold"),
    llvm::cl::init(120000));

static llvm::cl::opt<unsigned> cppShardThresholdBytes(
    "cpp-shard-threshold-bytes",
    llvm::cl::desc("Shard oversized C++ module sources when generated byte "
                   "count exceeds this threshold"),
    llvm::cl::init(4 * 1024 * 1024));

static llvm::cl::opt<unsigned> cppShardMaxAstNodes(
    "cpp-shard-max-ast-nodes",
    llvm::cl::desc("Chunk large eval topologies into helper methods (node "
                   "budget per chunk; 0=default policy)"),
    llvm::cl::init(0));

static llvm::cl::opt<std::string>
    cppManifestPath("cpp-manifest",
                    llvm::cl::desc("C++ compile manifest path (default: "
                                   "<out-dir>/cpp_compile_manifest.json)"),
                    llvm::cl::init(""));

static llvm::cl::opt<std::string> probeManifestPath(
    "probe-manifest",
    llvm::cl::desc(
        "Write DFX probe manifest JSON to this path (Decision 0100)"),
    llvm::cl::init(""));

static llvm::cl::opt<std::string>
    probePlanPath("probe-plan",
                  llvm::cl::desc("Resolved @probe alias plan JSON to embed "
                                 "into generated ProbeRegistry registration"),
                  llvm::cl::init(""));

static llvm::cl::opt<std::string>
    targetKind("target", llvm::cl::desc("Target: default|fpga"),
               llvm::cl::init("default"));

static llvm::cl::opt<bool>
    includePrims("include-primitives",
                 llvm::cl::desc("Emit `include` for PYC Verilog primitives"),
                 llvm::cl::init(true));

static llvm::cl::opt<std::string> outDir(
    "out-dir",
    llvm::cl::desc("Output directory (split per module; emits manifest.json)"),
    llvm::cl::init(""));

static llvm::cl::opt<unsigned>
    logicDepthLimit("logic-depth",
                    llvm::cl::desc("Maximum combinational logic depth allowed "
                                   "between sequential boundaries"),
                    llvm::cl::init(32));

static llvm::cl::opt<std::string>
    simMode("sim-mode", llvm::cl::desc("Simulation mode: default|cpp-only"),
            llvm::cl::init("default"));

static llvm::cl::opt<bool> cppOnlyPreserveOps(
    "cpp-only-preserve-ops",
    llvm::cl::desc("Preserve operation-granular C++ scheduling in "
                   "--sim-mode=cpp-only (disables comb fusion)"),
    llvm::cl::init(false));

static llvm::cl::opt<bool>
    noInline("noinline",
             llvm::cl::desc("Disable MLIR inliner to preserve module "
                            "boundaries (prevents merge/flatten)"),
             llvm::cl::init(false));
...
static llvm::cl::opt<std::string>
    hierarchyPolicy("hierarchy-policy",
...
static llvm::cl::opt<unsigned> canonicalizeBudget(
    "canonicalize-budget",
    llvm::cl::desc(
        "Canonicalizer budget (max iterations; 0 keeps profile default)"),
    llvm::cl::init(0));
...
static llvm::cl::opt<bool>
    profilePassTiming("profile-pass-timing",
                      llvm::cl::desc("Collect pass-level timing and memory "
                                     "stats (included in --profile-json)"),
                      llvm::cl::init(false));
...
static llvm::cl::opt<bool>
    clFlatten("flatten",
              llvm::cl::desc("Flatten all module hierarchy: inline all "
                             "pyc.instance ops into a single module"),
              llvm::cl::init(false));

static llvm::cl::opt<bool> clHierarchical(
    "hierarchical",
    llvm::cl::desc("Preserve module hierarchy: keep pyc.instance boundaries as "
...
static std::uint64_t hash64ForPathWithSuffix(llvm::StringRef path,
                                             std::uint32_t suffix) {
  std::string tmp;
...
static std::string getPortCanonicalFieldPath(func::FuncOp f, unsigned idx,
                                             bool isResult) {
  if (!isResult) {
...
static std::uint64_t assignProbeId(llvm::StringRef path,
                                   const ProbeManifestIndex &idx) {
  // Decision 0022: detect collisions and resolve by rehashing with "#<n>"
  // suffix.
  ...
  if (idx.by_id.count(id)) {
    ...
    llvm::errs() << "[pyc] Probe manifest hash collision: base=" << u64Hex(base)
                 << " path='" << path << "' resolved=" << u64Hex(id) << "\n";
  }
  return id;
}

static LogicalResult
writeProbeManifestJson(llvm::StringRef path, llvm::StringRef top,
...
static LogicalResult emitProbeManifest(ModuleOp module,
                                       llvm::StringRef outPath) {
  std::string top = topSymbol(module);
  if (top.empty()) {
    pyc::emitError(module, "PYC926")
        << "cannot emit probe manifest: missing `pyc.top`";
    return failure();
  }

  auto topFunc = module.lookupSymbol<func::FuncOp>(top);
  if (!topFunc) {
    pyc::emitError(module, "PYC927")
        << "cannot emit probe manifest: top symbol not found: " << top;
    return failure();
  }
  ...
  auto rec = [&](auto &&self, func::FuncOp f,
                 std::string instPath) -> LogicalResult {
    const std::string sym = f.getSymName().str();
    if (std::find(stack.begin(), stack.end(), sym) != stack.end()) {
      pyc::emitError(f, "PYC928") << "recursive instance hierarchy is not "
                                     "supported for probe manifest: "
                                  << sym;
      return failure();
    }
    stack.push_back(sym);
    ...
    if (auto hardenedAttr = f->getAttrOfType<StringAttr>("pyc.hardened")) {
      llvm::Expected<llvm::json::Value> parsed =
          llvm::json::parse(hardenedAttr.getValue());
      if (parsed) {
        if (auto *obj = parsed->getAsObject()) {
          if (auto *probeTable = obj->getObject("probe_table")) {
            for (const auto &kv : *probeTable) {
              ...
              probeMetaByPort.try_emplace(llvm::StringRef(kv.first),
                                          std::move(meta));
            }
          }
        }
    ...
    if (!f.isDeclaration()) {
      ...
      if (!ret) {
        pyc::emitError(f, "PYC931")
            << "probe manifest emission requires func.return terminator";
        return failure();
      }
      ...
      for (unsigned i = 0; i < f.getNumResults() && i < ret.getNumOperands();
           ++i)
    ...
    for (unsigned i = 0; i < f.getNumResults(); ++i) {
      ...
      if (auto it = probeMetaByPort.find(e.field_path);
          it != probeMetaByPort.end()) {
    ...
    if (!llvm::hasSingleElement(f.getBody()))
      return pyc::emitError(f, "PYC929")
             << "probe manifest emission requires single-block funcs (match "
                "C++ emitter contract)";
    ...
    // Decision 0006: mem probes exist and must have stable names (enforced by
    // frontend contract pass).
    const auto byteMems =
        collectMemNames([](Operation &op) { return isa<pyc::ByteMemOp>(op); });
    ...
    const auto syncMems =
        collectMemNames([](Operation &op) { return isa<pyc::SyncMemOp>(op); });
    ...
    const auto syncMemDPs = collectMemNames(
        [](Operation &op) { return isa<pyc::SyncMemDPOp>(op); });
    ...
    for (Operation &op : topBlock) {
      ...
      if (!nameAttr)
        return pyc::emitError(inst, "PYC930")
               << "missing required InstanceOp name (expected frontend "
                  "contract to enforce)";
      ...
      if (!callee)
        return inst.emitError("callee symbol not found: ")
               << calleeAttr.getValue();
      std::string seg = nameAttr.getValue().str();
      if (auto shortAttr = inst->getAttrOfType<StringAttr>("short_name"))
        seg = shortAttr.getValue().str();
      children.push_back(
          ChildInfo{std::string(nameAttr.getValue()), std::move(seg), callee});
    }
    std::sort(
        children.begin(), children.end(),
        [](const ChildInfo &a, const ChildInfo &b) { return a.name < b.name; });
  ...
  std::sort(entries.begin(), entries.end(),
            [](const ProbeManifestEntry &a, const ProbeManifestEntry &b) {
...
static FailureOr<llvm::json::Object> parseTbPayload(ModuleOp module) {
  ...
  if (!parsed || !parsed->getAsObject()) {
    pyc::emitError(module, "PYC920")
        << "invalid `pyc.tb.payload` JSON encoding";
    return failure();
  }
  return *parsed->getAsObject();
}

static LogicalResult emitTbTextFromPayload(ModuleOp module,
                                           llvm::StringRef kind,
                                           llvm::raw_ostream &os) {
  ...
  pyc::emitError(module, "PYC923")
      << "unsupported testbench emit kind `" << kind << "`";
  return failure();
}
...
static std::optional<std::string> findToolchainRoot(const char *argv0) {
  auto tryRoot =
      [&](llvm::SmallString<256> root) -> std::optional<std::string> {
    ...
    llvm::sys::path::append(cmakeCfg, "share", "pycircuit", "cmake",
  ...
  llvm::ErrorOr<std::string> resolved =
      llvm::sys::findProgramByName(argv0 ? argv0 : "");
...
collectSelectedRtlSources(ModuleOp module) {
  ...
  llvm::StringMap<std::string> licenses;
  WalkResult walk = module.walk([&](pyc::RtlCombOp selected) {
    ...
    for (Attribute raw : sources) {
      ...
      if (!path || !license)
        return WalkResult::interrupt();
      auto [entry, inserted] =
          licenses.try_emplace(path.getValue(), license.getValue());
      if (!inserted) {
        if (entry->getValue() != license.getValue())
          return WalkResult::interrupt();
        continue;
      }
      result.push_back({path.getValue().str(), license.getValue().str()});
    }
...
static LogicalResult emitPrimitivesFile(llvm::StringRef outPath,
  ...
  static const char *kFiles[] = {
      "pyc_reg.v",      "pyc_fifo.v",        "pyc_byte_mem.v",
      "pyc_sync_mem.v", "pyc_sync_mem_dp.v", "pyc_async_fifo.v",
      "pyc_cdc_sync.v",
  };
...
static LogicalResult
enforceCppCompileBudgets(ModuleOp module,
  ...
  if (topSourceCost > kHardMaxSourcePredictedCompileCost) {
    pyc::emitError(module, "PYC991")
        << "hottest emitted C++ TU exceeds hard budget: path=`" << topSourcePath
  ...
  if (topModuleCost > kHardMaxModulePredictedCompileCost) {
    pyc::emitError(module, "PYC992")
        << "hottest emitted module exceeds hard budget: module=`"
        << topModuleName << "` predicted_compile_cost=" << topModuleCost
        << " > " << kHardMaxModulePredictedCompileCost;
    ok = false;
  }
  if (totalCost > kHardMaxTotalPredictedCompileCost) {
    pyc::emitError(module, "PYC993")
        << "total emitted C++ compile cost exceeds hard budget: "
...
static uint64_t computeComplexityScore(uint64_t lines, uint64_t bytes,
                                       llvm::StringRef kind) {
...
static double computePredictedCompileCost(uint64_t complexityScore,
                                          llvm::StringRef kind) {
  double w = 1.0;
  if (kind == "tick")
    w = 1.45;
...
private:
  ...
  void finalize(mlir::Pass *pass) {
    ...
    uint64_t ms = static_cast<uint64_t>(
        std::chrono::duration_cast<std::chrono::milliseconds>(now - a.start)
            .count());
...
static std::string removeDefaultArgs(llvm::StringRef sig) {
  ...
  for (size_t i = open + 1; i < close; ++i) {
    ...
    if (!skipping) {
      ...
      if (c == '=' && depthParen == 0 && depthAngle == 0 && depthBrace == 0 &&
          depthBracket == 0) {
        skipping = true;
        continue;
      }
...
static std::string methodKindForName(llvm::StringRef shortName) {
  if (shortName.starts_with("eval_comb_"))
    return "comb";
  if (shortName == "eval" || shortName == "eval_comb_pass" ||
      shortName.starts_with("eval_"))
    return "eval";
...
static std::optional<SplitMethodDef>
convertMethodChunk(llvm::StringRef chunk, llvm::StringRef structName,
                   std::string &error) {
  ...
  std::string fullName =
      trimCopy(llvm::StringRef(defSig).substr(nameStart, nameEnd - nameStart));
  if (fullName.empty()) {
...
static bool isMethodStartLine(llvm::StringRef line,
                              llvm::StringRef structName) {
  std::string t = trimCopy(line);
  if (t.empty())
    return false;
  if (startsWithWord(t, "struct") || startsWithWord(t, "class") ||
      startsWithWord(t, "enum"))
    return false;
  if (t.find('(') == std::string::npos)
    return false;
  if (t.size() >= 2 && t[0] == '/' && t[1] == '/')
    return false;
  if (!(startsWithWord(t, "inline") || startsWithWord(t, "void") ||
        startsWithWord(t, "static") || startsWithWord(t, "bool") ||
        startsWithWord(t, structName) ||
        startsWithWord(t, "~" + structName.str()))) {
    return false;
  }
  size_t semi = t.find(';');
  size_t brace = t.find('{');
...
static std::optional<SplitModuleResult>
splitEmittedModule(llvm::StringRef emitted, llvm::StringRef structName,
                   std::string &error) {
    ...
    if (isMethodStartLine(line, structName)) {
      ...
      if (lineEnd != std::string::npos && methodOpen > lineEnd) {
        ...
        while (methodOpen == std::string::npos && search < body.size()) {
          ...
          if (methodOpen != std::string::npos &&
              (le == std::string::npos || methodOpen <= le))
            break;
          search = nx;
        }
      }
...
static LogicalResult
writeCppCompileManifest(llvm::StringRef path, llvm::StringRef targetName,
                        llvm::ArrayRef<CppManifestSource> sources,
...
static CompileStatsSummary collectCompileStats(ModuleOp module,
                                               int64_t depthLimit) {
  ...
  for (auto f : module.getOps<func::FuncOp>()) {
    ...
    s.maxLogicDepth =
        std::max(s.maxLogicDepth, getI64Attr(f, "pyc.logic_depth.max", 0));
...
static LogicalResult writeCompileStatsJson(llvm::StringRef outPath,
                                           const CompileStatsSummary &s) {
  llvm::json::Object obj;
...
static LogicalResult writeProfileJson(llvm::StringRef outPath,
...
int main(int argc, char **argv) {
  ...
  if ((hasDirectCpp || hasDirectVerilog) && !outDir.empty()) {
    llvm::errs() << "error: direct output mode (-cpp/-verilog) cannot be "
                    "combined with --out-dir\n";
    return 1;
  }
  if ((hasDirectCpp || hasDirectVerilog) && outputFilename.empty()) {
    llvm::errs()
        << "error: direct output mode requires a non-empty output path\n";
    return 1;
  }
  ...
  if (profileJsonPathResolved.empty() && profilePassTiming) {
    if (!outDir.empty()) {
      ...
      profileJsonPathResolved =
          (llvm::Twine(outputFilename) + ".profile.json").str();
    }
  }
  const bool collectPassTiming =
  ...
  if (getTbPayloadAttr(*module).has_value()) {
    if (!outDir.empty()) {
      llvm::errs() << "error: testbench payload mode does not support "
                      "--out-dir; use direct -cpp/-verilog or -o\n";
      return 1;
    }
    std::error_code ec;
  ...
  if (simMode == "default") {
    ...
    llvm::errs() << "error: unknown --sim-mode: " << simMode
                 << " (expected: default|cpp-only)\n";
    return 1;
  }

  if (emitStructuralMode != "auto" && emitStructuralMode != "on" &&
      emitStructuralMode != "off") {
    llvm::errs() << "error: unknown --emit-structural: " << emitStructuralMode
  ...
  if (wantFlatten && wantHierarchical) {
    llvm::errs()
        << "error: --flatten and --hierarchical are mutually exclusive\n";
    return 1;
  }
  ...
  const std::set<std::string> moduleSymbolsBefore =
      collectFrontendModuleSymbols(*module);
  ...
  if (forceNoInline) {
    ...
    llvm::StringRef n =
        llvm::StringRef(inlinePolicy).drop_front(strlen("threshold:"));
    uint64_t threshold = 0;
    if (n.getAsInteger(10, threshold) || threshold == 0) {
      llvm::errs() << "error: invalid --inline-policy threshold, expected "
    ...
    inlineDecision.reason = inlineDecision.enableInline
                                ? "module-op-count <= threshold"
                                : "module-op-count > threshold";
  } else {
  ...
  if (!wantFlatten && (strictHierarchy || instantiateHierarchy) &&
      inlineDecision.enableInline) {
    ...
    llvm::errs() << "  inline_policy=" << inlineDecision.mode
                 ...
                 << ")\n";
    llvm::errs() << "  hint: use --inline-policy=off (or "
                    "--inline-policy=default with --hierarchy-policy="
  ...
  passMs = static_cast<uint64_t>(
      std::chrono::duration_cast<std::chrono::milliseconds>(Clock::now() -
                                                            tPassStart)
          .count());

  const std::set<std::string> moduleSymbolsAfter =
      collectFrontendModuleSymbols(*module);
  const bool hierarchyPreserved = (moduleSymbolsBefore == moduleSymbolsAfter);
  if (strictHierarchy && !wantFlatten && !hierarchyPreserved) {
    llvm::errs() << "error: strict hierarchy policy violation: frontend module "
                    "symbol set changed after passes\n";
  ...
  if (instantiateHierarchy && !wantFlatten && !hierarchyPreserved) {
    llvm::errs() << "warning: instantiate hierarchy: some module symbols "
                    "changed after passes\n";
    for (const auto &sym : moduleSymbolsBefore) {
      if (!moduleSymbolsAfter.count(sym))
        llvm::errs() << "  removed_module=" << sym << "\n";
    }
  ...
  CompileStatsSummary compileStats =
      collectCompileStats(*module, static_cast<int64_t>(logicDepthLimit));
  ...
  auto buildProfileSummary = [&]() -> llvm::json::Object {
    ...
    obj["module_count_before"] =
        static_cast<int64_t>(moduleSymbolsBefore.size());
    ...
    obj["inline_threshold_ops"] =
        static_cast<int64_t>(inlineDecision.thresholdOps);
    obj["inline_reason"] = inlineDecision.reason;
    obj["canonicalize_budget"] =
        static_cast<int64_t>(effectiveCanonicalizeBudget);
    obj["module_op_count"] = static_cast<int64_t>(moduleOpCount);
    obj["cpp_shard_threshold_lines"] =
        static_cast<int64_t>(cppShardThresholdLines);
    obj["cpp_shard_threshold_bytes"] =
        static_cast<int64_t>(cppShardThresholdBytes);
  ...
  auto writeProfileArtifact = [&](llvm::StringRef lane) -> LogicalResult {
    ...
    hierarchyObj["module_count_before"] =
        static_cast<int64_t>(moduleSymbolsBefore.size());
    hierarchyObj["module_count_after"] =
        static_cast<int64_t>(moduleSymbolsAfter.size());
    hierarchyObj["preserved"] = hierarchyPreserved;
    root["hierarchy"] = std::move(hierarchyObj);
    root["canonicalize_budget"] =
        static_cast<int64_t>(effectiveCanonicalizeBudget);
  ...
  if (!outDir.empty()) {
    ...
    if (ec) {
      llvm::errs() << "error: cannot create --out-dir " << outDir << ": "
                   << ec.message() << "\n";
      return 1;
    }
    ...
    std::string top = topSymbol(*module);
    if (top.empty()) {
      llvm::errs() << "error: cannot determine top symbol (missing pyc.top and "
                      "no func.func)\n";
      return 1;
    }
    ...
    if (emitKind == "verilog") {
      if (cppOnly) {
        llvm::errs() << "error: --emit=verilog is not allowed with "
                        "--sim-mode=cpp-only\n";
        return 1;
      }
      ...
      if (!targetFpga && targetKind != "default") {
        llvm::errs() << "error: unknown --target: " << targetKind
                     << " (expected: default|fpga)\n";
        return 1;
      }
      if (includePrims) {
        auto primDir = findPrimitivesDir(argv[0]);
        if (!primDir) {
          llvm::errs() << "error: cannot locate library/verilog for "
                          "primitives; set PYC_PRIMITIVES_DIR\n";
          return 1;
        }
      ...
      opts.includePrimitives = false; // out-dir mode uses pyc_primitives.v (or
                                      // expects external primitives)
    ...
    if (emitKind == "cpp") {
      ...
      if (cppSplitMode == "module")
        ...
        llvm::errs() << "error: unknown --cpp-split mode: " << cppSplitMode
                     << " (expected: module|none)\n";
        return 1;
      }

      auto writeHeaderPreamble = [&](llvm::raw_ostream &os,
      ...
      auto writeSourcePreamble = [&](llvm::raw_ostream &os,
      ...
      auto writeSourceEpilogue = [&](llvm::raw_ostream &os) {
        os << "} // namespace pyc::gen\n";
      };
      ...
      for (const ModuleEmitRequest &request : moduleRequests) {
        ...
        if (splitModule) {
          ...
          if (!split) {
            llvm::errs() << "error: failed to split module `" << moduleName
                         << "`: " << splitErr << "\n";
            return 1;
          }
          ...
          if (fe) {
            llvm::errs() << "error: cannot open " << headerPath << ": "
                         << fe.message() << "\n";
            return 1;
          }
          ...
          auto writeSourceFile =
              [&](llvm::StringRef fileName, llvm::StringRef kind,
            ...
            if (se) {
              llvm::errs() << "error: cannot open " << srcPath << ": "
                           << se.message() << "\n";
              return failure();
            }
            ...
            if (!srcOrErr) {
              llvm::errs() << "error: cannot read generated source " << srcPath
            ...
            info.complexityScore =
                computeComplexityScore(info.lines, info.bytes, info.kind);
            info.predictedCompileCost =
                computePredictedCompileCost(info.complexityScore, info.kind);
          ...
          if (!oversized) {
            if (failed(writeSourceFile(moduleName + ".cpp", "module", "000",
                                       allMethods)))
              return 1;
          } else {
            bool wroteAny = false;
            if (failed(writeSourceFile(moduleName + "__core.cpp", "core",
                                       "core", coreMethods)))
              return 1;
            if (!coreMethods.empty())
              wroteAny = true;
            auto shardPredictedCost = [&](size_t lines, size_t bytes,
                                          llvm::StringRef kind) -> double {
              return computePredictedCompileCost(
                  computeComplexityScore(static_cast<uint64_t>(lines),
                                         static_cast<uint64_t>(bytes), kind),
                  kind);
            };
            ...
            auto writeMaybeShardedMethods =
                [&](llvm::StringRef stem, llvm::StringRef kind,
                    llvm::ArrayRef<const SplitMethodDef *> defs,
              ...
              double totalPredictedCostLocal =
                  shardPredictedCost(totalLinesLocal, totalBytesLocal, kind);
              bool needsSharding =
                  allowSharding && defs.size() > 1 &&
                  ((totalBytesLocal >
                    static_cast<size_t>(cppShardThresholdBytes)) ||
                   (totalLinesLocal >
                    static_cast<size_t>(cppShardThresholdLines)) ||
                   (totalPredictedCostLocal >
                    kShardSoftMaxSourcePredictedCompileCost));

              if (!needsSharding) {
                if (failed(
                        writeSourceFile(moduleName + "__" + stem.str() + ".cpp",
                                        kind, stem, defs)))
                  return failure();
                wroteAny = true;
                return success();
              ...
              auto flushShard = [&]() -> LogicalResult {
                ...
                if (failed(writeSourceFile(moduleName + "__" + stem.str() +
                                               "_" + shardNum + ".cpp",
              ...
              for (const SplitMethodDef *m : defs) {
                double projectedCost = shardPredictedCost(
                    shardLines + m->lines, shardBytes + m->bytes, kind);
                bool limitHit =
                    !shardDefs.empty() &&
                    ((shardBytes + m->bytes >
                      static_cast<size_t>(cppShardThresholdBytes)) ||
            ...
            if (failed(writeMaybeShardedMethods("tick", "tick", tickMethods,
                                                /*allowSharding=*/true)))
              return 1;

            if (failed(writeMaybeShardedMethods("eval", "eval", evalMethods,
                                                /*allowSharding=*/true)))
              return 1;
            if (failed(writeMaybeShardedMethods("comb", "comb", combMethods,
                                                /*allowSharding=*/true)))
              return 1;
          ...
          hos << "// Generated by pyCircuit C++ emission.\n";
          hos << "#pragma once\n";
          hos << "#include <cstdlib>\n";
      ...
      if (splitModule) {
        ...
        if (!cppManifestPath.empty()) {
          ...
          llvm::sys::path::append(manifestPathStorage,
        ...
        manifestProfile["pycc_peak_rss_bytes"] =
            static_cast<int64_t>(getPeakRssBytes());
        manifestProfile["pass_time_ms"] = static_cast<int64_t>(passMs);
        auto toolchainRoot = findToolchainRoot(argv[0]);
        if (failed(writeCppCompileManifest(
                manifestPathStorage, top, cppManifestSources, includeDirs,
                compileDefines, toolchainRoot, topHeaderName, manifestProfile)))
          return 1;
      }

      if (failed(updateManifest(outDir, top, /*verilogMods=*/std::nullopt,
                                std::move(cppFiles))))
        return 1;
  ...
  if (emitKind == "verilog") {
    if (cppOnly) {
      llvm::errs()
          << "error: --emit=verilog is not allowed with --sim-mode=cpp-only\n";
      return 1;
    }
    pyc::VerilogEmitterOptions opts;
