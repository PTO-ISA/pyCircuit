...
def _safe_uint(
    value: object, *, line: int, pointer: str, sequence: bool = False
) -> int:
...
def _tile(
    ...
    for index, dimension in enumerate(shape):
        decoded = _safe_uint(dimension, line=line, pointer=f"{pointer}/shape/{index}")
        if decoded == 0:
            _fail(
                "ACTRACE-ADAPTER-SCHEMA",
    ...
    return {
        ...
        "dtype": _string(tile["dtype"], limits, line=line, pointer=pointer + "/dtype"),
    }
...
def convert_davincioo_trace(
    ...
    *,
    ...
    if source_program is not None and (
        type(source_program) is not str or not source_program
    ):
        _fail(
            "ACTRACE-ADAPTER-SCHEMA",
