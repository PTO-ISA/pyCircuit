#!/usr/bin/env python3
"""
Register-file runtime benchmark for the 256-entry, 10R/5W integration fixture.
...
class RegFileRTL:
    def __init__(self, lib_path: str | None = None):
        ...
        library.rf_create.restype = ctypes.c_void_p
        library.rf_destroy.argtypes = [ctypes.c_void_p]
        library.rf_reset.argtypes = [ctypes.c_void_p, ctypes.c_uint64]
        library.rf_drive_read.argtypes = [
            ctypes.c_void_p,
            ctypes.c_uint32,
            ctypes.c_uint8,
        ]
        library.rf_drive_write.argtypes = [
            ctypes.c_void_p,
            ctypes.c_uint32,
            ctypes.c_uint8,
            ctypes.c_uint8,
        ...
        library.rf_tick.argtypes = [ctypes.c_void_p, ctypes.c_uint64]
        library.rf_get_rdata.argtypes = [ctypes.c_void_p, ctypes.c_uint32]
        library.rf_get_rdata.restype = ctypes.c_uint64
        library.rf_get_cycle.argtypes = [ctypes.c_void_p]
        library.rf_get_cycle.restype = ctypes.c_uint64
        library.rf_run_bench.argtypes = [ctypes.c_void_p, ctypes.c_uint64]
        library.rf_run_bench_cd.argtypes = [
            ctypes.c_void_p,
            ctypes.c_uint64,
            ctypes.c_uint32,
        ]
...
def main():
    ...
    cycles = 100_000
    print(
        f"{BOLD}Performance Benchmark "
        f"({cycles // 1000}K cycles, 100% active){RESET}"
    )
    print("  Mixed random read/write traffic per cycle...")

    elapsed = benchmark(rf, cycles)
    khz = cycles / elapsed / 1000
    print(f"\n  Cycles:    {cycles:>12,}")
    print(f"  Elapsed:   {elapsed:>12.4f} s")
    print(f"  Throughput:{khz:>12.1f} Kcycles/s")
    print(f"  Per cycle: {elapsed / cycles * 1e6:>12.2f} us")

    # ── Benchmark: change-detection with varying activity rates ──
    print(f"\n{BOLD}Change-Detection Benchmark " f"({cycles // 1000}K cycles){RESET}")
    for pct in [100, 50, 25, 10, 1]:
        rf.reset()
        rf.run_bench_cd(1000, pct)  # warm up
        t0 = time.perf_counter()
        rf.run_bench_cd(cycles, pct)
        t1 = time.perf_counter()
        el = t1 - t0
        kc = cycles / el / 1000
