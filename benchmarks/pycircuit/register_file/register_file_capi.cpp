 ...
 *       -o .pycircuit_out/benchmarks/register_file/libregfile_sim.dylib \
 *       benchmarks/pycircuit/register_file/register_file_capi.cpp
 */
...
using pyc::cpp::InputChangeTracker;
using pyc::cpp::Wire;
...
struct SimContext {
  pyc::gen::RegFile dut{};
  pyc::cpp::Testbench<pyc::gen::RegFile> tb;
  uint64_t cycle = 0;

  InputChangeTracker<80, 5, 40, 320> input_tracker;
  bool eval_dirty = true;

  SimContext()
      : tb(dut), input_tracker(dut.raddr_bus, dut.wen_bus, dut.waddr_bus,
                               dut.wdata_bus) {
  ...
  void eval_if_dirty() {
    if (eval_dirty || input_tracker.check_and_capture()) {
      dut.eval();
      eval_dirty = false;
    }
  }
...
void rf_drive_write(SimContext *c, uint32_t lane, uint8_t en, uint8_t addr,
                    uint64_t data) {
  ...
  for (uint64_t i = 0; i < n_cycles; i++) {
    ...
    if (active) {
      ...
      for (unsigned j = 8; j < NR; j++)
        w1 |= (uint64_t)((uint8_t)((r >> (j * 2)) & 0xFF))
              << ((j - 8) * PTAG_W);
