...
>
> **Agentic Circuit 边界**：Decisions 0151–0152 将独立的 Agentic Circuit 契约保持在 epoch `0.4`，并加入只由 typed gfsim C++ 支持的一维状态 Table、committed slot、match/choose 与 state-driven update 原型。它们不改变 CycleAwareSignal 或 PYC 的语义；PYC/RTL 遇到这些 provisional state op 必须在明确边界拒绝。
