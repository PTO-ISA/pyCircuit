...
- Decision 0281 freezes one typed ordering relation and one closed load
  ...
  identity, stale-set, or tracking mechanism.
- `ac.memory_order_edge` carries producer and consumer lane masks, an optional
  ...
  must_forward, must_replay_if, visibility_before}`. `must_wait` dominates
  `may_bypass`.
...
- The new closed obligation kind `no_stale_response` is emitted once per
  disposition with the stable ID `no_stale_response:<anchor>`. PYC, generated
  C++, and RTL carry the same ID, kind, severity, `pre_publish` sampling point,
  anchor, and message; C++ gets a `_coverage` counter and RTL a cover property
  for the stale event.

## Fixture and oracle

The reduced fixture executes in generated C++ and in Icarus RTL. Its
independent oracle does not restate the DUT expression: it derives `alias`,
`disjoint`, and `data_ready` from a real address and store model (an alias
requires a resolved older store at the same address, and a disjoint proof
requires every older store to be resolved and at a different address), and it
models a flush event that invalidates outstanding loads. Each lane is asserted
to have exactly one disposition, and the fixture fails when any disposition
never occurs.
...
- The gfsim C++ obligation path computes the stale predicate and the plan
  ...
  the matched assertion and coverage, as in the earlier stages.
