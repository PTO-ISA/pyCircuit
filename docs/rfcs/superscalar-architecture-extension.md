# RFC: Superscalar 架构扩展 —— 面向乱序 Core 的 Architecture Rule Compiler
...
**Primary validation workload:** Superscalar NPU Core 的 reduced fixture（完整模型留在 consumer repository）
...
本文提出 pyCircuit 的下一代扩展方向：把「架构意图」提升为编译器可推理的一等契约，而不是继续往
framework 里堆 CPU/NPU 专用 primitive。目标能力是从
...
- 需求 A（Rule Effect Graph）所需的**单 rule 效果摘要与 Table writer 冲突仲裁已经存在**，
  而且 §5.5 的验收判据在 Table field writer 范围内**已经满足**；
- 需求 B（Obligation IR）所需的**生命周期骨架已经存在**，缺的是 `checks` resolver 的实现；
- 真正的缺口集中在**恢复语义（C/S/J）、多 lane 事务代数（D/E/F/G/H）、内存序（I）与
...
Superscalar Core 的困难不在单个 module，而在同一拍内数百个持久状态对象、数十条并行 pipeline
与大量 transaction rule 之间的状态一致性。用「一个 Queue 接到另一个 Queue」的方式描述这类设计，
会把架构规则散落到 ROB、IQ、Rename、LSU、pipeline、assertion 和 testbench 里，并且无法由编译器
统一保证一致性。
...
流水线与部分架构状态，但不足以让编译器回答「哪些 rule 会写同一个 Entry」「flush 后旧 completion
能否污染新 Entry」这类跨 rule、跨 owner 的问题。

因此下一阶段的产出不应是若干新 HDL primitive，而应是一层 **Architecture Rule Compiler**：
让架构师只描述「什么条件下、哪个 architecture transaction、允许修改什么 architecture state」，
其余由编译器推导并保持各实现层一致。
...
- 支持多 lane、多资源、原子预留的事务代数。
- 为恢复与投机提供统一的 epoch/generation/attempt 身份。
- 让 PPA 相关实现选择（banking、port、pipeline cut）成为可验证的 refinement，而非隐含行为。
- 让 agent 能直接查询「某条 rule 为什么没 fire」，而不是从 waveform 反推。

## 非目标

- **不在 framework 中加入 CPU/NPU 专用 API**：`ac.rob()`、`ac.rename_table()`、`ac.cpu_issue_queue()`、
  `ac.npu_lsu()` 一律不做。完整 ROB、IQ、LSU、Rename 属于 consumer design。
- **不把完整 Superscalar Core 放进本仓库**。AGENTS.md 与决策 0158、0235 规定：完整 CPU/NPU/SoC/board
...
- **不追求一次做完**。§26 的六个阶段必须顺序推进，10-wide 不得先于 4-wide。
...
| 动态索引值域证明 | `lib/Transforms/VerifyValueConstraints.cpp:369-501`；`lib/Transforms/VerifyValueConstraints.cpp:55-113` |
...
**关键结论：需求 §5.5 的验收判据在 Table field writer 范围内已经实现。**
...
| G AgeSelect-K | `TableSelectionPolicy`、`priority_encode`/`onehot_encode`、`.choose` round-robin cursor、`argmin` | 无「N 选 K 且满足资源约束」的语义原语；无 FU class 约束选择 |
...
| Q Retained Result | Slot 提交语义保留 payload（`docs/acir/spec/agentic-circuit.md:1006`） | 没有独立 primitive；无 flush-aware 资格 |
...
| ExecutionAttempt | 0 命中；示例 ROB 的 `attempt` 语义是用户字段 |
| KillSet / invalidation | `kill`、`invalidat` 命中 0 |
| Checkpoint / rollback | `checkpoint`、`rollback` 命中 0 |
| replay / nuke / squash | 命中 0 |
| TransactionGroup / ValidPrefix | `prefix accept` 命中 0；`ac-resolve-rule-schedule` 要求每个输入 Queue 独占（`LowerRules.cpp:608-619`），因此多 lane 原子多资源 dispatch **当前不可表达** |
...
| MemoryOrderEdge / LSQ / forwarding | 命中 0 |
| Why-not-fire | 命中 0；trace 无 rule/guard 事件 |
| 架构级 coverage manifest | 只有 IR/op 覆盖；`cover property` 全树命中 0 |
| NDF rule manifest | NDF 目前是**文档 profile**（`tools/agentic-circuit/check-ndf.py`），不是 rule manifest |
...
| `epoch` | ① `ac.contract_epoch = "0.5"` 工具链契约版本；② `gfsim::Epoch` 仿真时钟 | `_contract.py:3`；`_queue_codegen.py:1209` | 恢复 epoch 必须显式命名，例如 `recovery_epoch`，禁止裸 `epoch` |
...
| `refine` | capture-only marker `ac.refine` | `python/agentic-circuit/src/agentic_circuit/markers.py:132` | §15 的 `@ac.refine(...)` **与现有 marker 同名，必须改名或复用** |
...
自动携带 epoch；引入 `TransactionRef { slot, generation, epoch }`、recovery event、
...
**差距。** 这是 Superscalar 模型最明显的 framework 缺口，也是本文的**最高优先级**之一。
需要新增：recovery domain 声明、transaction 身份中的 epoch 分量、
以及跨 ROB/BROB/IQ/LSQ/retained result/engine response 的统一 kill predicate。

**阶段。** Phase 3（与 Requirement S 合并推进，因为两者都依赖身份模型）。

**验收（取自需求 §7.7）。** T0 在 slot 3、gen0、epoch4 分配；flush 到 epoch5；
T1 在 slot 3、gen1、epoch5 分配；旧 T0 的 completion 返回时，必须证明 T1 未受影响，
并生成对应的运行期 assertion。

## Requirement D：Multi-Lane Transaction Algebra

**需求。** 支持 8–10 发射所需的同拍多事务：`TransactionGroup<N>`、valid mask、lane identity，
以及 AllOrNone / ValidPrefix / Independent 三种 commit 语义；
D1 decode → D2 preview → D3 atomic multi-resource reservation 的通用事务模式。
...
这正是「ROB allocate 4 / IQ allocate 3 / Rename allocate 4」类分歧的根源。

**阶段。** Phase 4。必须先 4-wide 通过，再 8-wide，最后才是 10-wide。

## Requirement E：MultiAllocator

**需求。** generic 的多分配器原语：`ac.allocator(entries=, width=, ports=)`，
支持 allocate K / free K / 同拍 free+allocate / generation tracking / prefix allocate / priority allocate，
并把 `reuse_same_cycle` 变成显式 policy；自动生成 double allocate / double free / 占用上界等检查。
...
```python
table = ac.versioned_table[256, RobEntry](
    generation_bits=2,
    recovery=recovery,
)
```
...
避免 consumer 反复手写 `valid && slot && generation && epoch`。
...
`ac.age_select(candidates, count=4, order=ac.oldest_first)`，
并支持 resource class 约束（2 ALU / 1 BRU / 2 LSU / 1 CUBE），
physical realization（linear / balanced / bank-local / age-matrix / segmented）由 refinement 决定。
...
与资源匹配（依赖 Requirement E 的 FU class）。

**阶段。** Phase 4。

## Requirement H：Dependency Set

**需求。** 运行期依赖跟踪向量 `ac.dependency_set(entries=, identity=stid)`，
支持 add/resolve/`resolve_if(predicate)`/`is_ready`/kill，
且 resolve **必须**以 `{slot, generation, epoch}` 限定。
...
**差距。** 需要运行期、带 generation 限定的 resolve 语义；否则旧 Store 的 resolve 会污染新事务。

**阶段。** Phase 5。

## Requirement I：Memory Ordering Contract

**需求。** 不做完整 LSQ primitive，先做 Memory Ordering Graph：
...
**现状。** 完全缺失（`MemoryOrderEdge`、store resolve、forwarding、replay 命中均为 0）。
现有 `ac.memory` 只有单读单写、init 必须为 0、positive latency 的受限契约
（`_queue_compiler/memory_statements.py:200-260`）。

**差距。** 这是 pyCircuit 进入高复杂 Core 的一道真实门槛。第一阶段只做 Core-local ordering，
不碰完整 ISA memory consistency model。
...
API 描述架构语义，实现可以是 full snapshot、delta log、MapQ 或 history buffer。
...
例如 width ≤ 8 用简单实现、width ≤ 32 用 balanced tree、issue_width > 2 用 banked。
...
运行期支持 `--explain-rule <id>`，trace 产出 blocker JSON。
...
扩展为完整 blocker 分类。对 agent 而言这是收益最高的能力之一。
...
## Requirement O：NDF Rule Manifest

**需求。** 每个 rule 有 machine-readable manifest（id / name / intent / owners / inputs / effects /
recovery_domain / requirements / obligations），compiler 自动生成 `docs/generated/rules/` 文档。
...
**差距。** NDF 目前是**规范来源**，而需求 §19 要求的是**rule 级 manifest**。
两者不能混用同一套名字，否则会破坏 `check-ndf.py` 的 fail-closed 语义。
建议：rule manifest 使用独立命名空间与独立校验工具，并在文档中显式声明
"Markdown 不是 semantic authority，Python rule 才是"。
...
（address / data / side effect），只有 terminal effect 成功后才置 `ROB.done = true`，
以避免 partial completion，尤其是在 flush/replay/nuke 交叉时。
...
**需求。** variable latency engine 的 response 不能丢：accept once、hold until consumed、
preserve transaction identity、flush-aware。
...
**差距。** 需要独立 primitive，并具备 recovery 资格判定。注意 Slot 的既有纪律已经预演了
...
**差距。** 需要统一的身份层级与归属判定。framework 不应知道 RID/BID/STID/PTAG/TTAG/UTAG 这些业务名字。

**阶段。** Phase 3。

## Requirement S：Speculative Execution Attempt

**需求。** 同一 instruction 的多次 issue/replay 需要 `ExecutionAttempt`，
身份为 `{RID, generation, epoch, attempt}`，旧 attempt 的 response 必须失效。
...
- 验收 workload：ROB、simple IQ、rename map。
...
- 验收 workload：branch mispredict 下的 ROB / Rename / IQ / completion 全链恢复。
- 完成标志：旧 completion 不能污染新 state，且该性质有运行期 assertion。

### Phase 4：Wide Superscalar
...
- 顺序：4-wide → 8-wide → 10-wide。

### Phase 5：LSU and Memory Ordering

- DependencySet、Tracking relation、MemoryOrderEdge、store resolve、forwarding、replay identity。
- 验收 workload：load behind unresolved store、non-alias resolution、alias forwarding、
  late violation、flush during outstanding load、old response after replay。
...
## Superscalar Acceptance Demo

整个扩展最终需要一个 framework 级 demo，但**不是完整 NPU**。建议 MiniOOO：

```text
4-wide dispatch / 32-entry ROB / 16-entry IQ / 32 physical tags
2 ALU / 1 BRU / versioned completion / branch recovery / simple load dependency
支持 dispatch, rename, issue, OoO execute, completion, retire, branch flush
```

验收方式：随机跑 1M+ transactions，验证 C++ / Verilator parity、zero-loss、precise retire、
flush correctness、stale response rejection、allocator integrity、ROB ordering、IQ no-double-issue。
Python 架构模型建议控制在 2,000 meaningful LOC 以内——重点不是硬卡 LOC，
而是验证抽象是否真的有效。
...
待 semantic contract 稳定后再公开 Python API。公开前必须先解决 §「命名冲突与语义归属」中的每一项，
尤其是 `refine`、`epoch`、`reservation`、`invariant`、`dependency` 五个已占用的拼写。
...
**Recovery**
- branch flush / replay / stale completion / slot reuse / epoch transition

**Wide machine**
- multi-dispatch / multi-allocation / multi-issue / multi-completion / multi-retire

**LSU**
- load-store dependency / forwarding / replay / load generation / stale response
...
而在于原来散落在 ROB、IQ、Rename、LSU、pipeline、assertion 和 testbench 中的同一条架构规则，
...
- bare-metal epoch 与 `ac.contract_epoch`/`gfsim::Epoch` 的命名空间如何划分？
