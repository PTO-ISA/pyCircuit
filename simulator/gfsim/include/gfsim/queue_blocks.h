...
public:
  ...
  const auto &committedValues() const { return committed_; }
  const Entry &at(size_t index) const {
    const auto &value = committed_.at(index);
    if (stateObserver_)
      stateObserver_->notify({StateAction::TableRead, id(), index, &value});
    return value;
  }
    ...
    for (TableWriteMode mode :
      ...
      for (const auto &[writerId, proposal] : pending_) {
        ...
        auto commitValue = [&](size_t index, const Entry &value) {
          if (stateObserver_)
            stateObserver_->notify({StateAction::BeforeWrite, id(), index,
                                    &committed_[index], 0, writerId,
                                    proposal.footprint.fields,
          ...
          if (stateObserver_)
            stateObserver_->notify({StateAction::AfterWrite, id(), index,
                                    &committed_[index], 0, writerId,
                                    proposal.footprint.fields,
                                    static_cast<uint8_t>(mode)});
        };
