    ...
    *,
    ...
    def drive_expr_lines(sn: str, expr: str, ty: str, *, indent: str) -> list[str]:
        ...
        if info.total_width > 64:
            raise SystemExit(
                f"random() for i{info.total_width} is not supported by the C++ testbench generator"
            )
    ...
    def packed_value_expr(sn: str, ty: str) -> str:
        ...
        if info.total_width > 64:
            raise SystemExit(
                f"print() for i{info.total_width} is not supported by the C++ testbench generator"
            )
    ...
    for p in getattr(t, "prints", []):
        ...
        for raw in p.ports:
            ...
            if w > 64:
                raise SystemExit(
                    f"print() for i{w} is not supported by the C++ testbench generator"
                )
            port_specs.append((str(raw), sn, w))
        if p.at is not None:
            prints_at.setdefault(int(p.at), []).append((fmt, port_specs))
    ...
    if t.random_streams:
        ...
        for r in t.random_streams:
            ...
            if w > 64:
                raise SystemExit(
                    f"random() for i{w} is not supported by the C++ testbench generator"
                )
    ...
    lines: list[str] = []
    lines.append("// Generated by pyCircuit.\n")
...
def _render_tb_sv(
    ...
    def sv_drive_expr_lines(name: str, expr: str, ty: str, *, indent: str) -> list[str]:
        ...
        if info.total_width > 64:
            raise SystemExit(
                f"random() for i{info.total_width} is not supported by the SystemVerilog testbench generator"
            )
    ...
    if t.random_streams:
        ...
        for r in t.random_streams:
            ...
            if w > 64:
                raise SystemExit(
                    f"random() for i{w} is not supported by the SystemVerilog testbench generator"
                )
            rand_specs.append((sn, w, ty, int(r.seed), int(r.start), int(r.every)))
