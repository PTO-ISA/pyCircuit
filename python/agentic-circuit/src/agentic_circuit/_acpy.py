    ...
    def verify(self) -> tuple[Diagnostic, ...]:
        ...
        for entity in self.entities:
            if entity.kind not in _ENTITY_KINDS:
                errors.append(
                    self._diagnostic("ACPy entity kind is invalid", entity.source)
                )
            if not entity.scope:
                errors.append(
                    self._diagnostic("ACPy entity scope is empty", entity.source)
                )
            if entity.source is not None and entity.source.file not in source_paths:
                errors.append(
                    self._diagnostic(
                        "ACPy entity source does not resolve", entity.source
                    )
                )
            references = [entity.parent, entity.definition, *entity.uses]
            if any(
                reference is not None and reference not in entity_ids
                for reference in references
            ):
                errors.append(
                    self._diagnostic(
                        "ACPy entity reference does not resolve", entity.source
                    )
                )
            if len(entity.uses) != len(set(entity.uses)):
                errors.append(
                    self._diagnostic("ACPy uses must be unique", entity.source)
                )
            ...
            if property_names != sorted(property_names, key=utf16_sort_key) or len(
                ...
                errors.append(
                    self._diagnostic(
                        "ACPy properties must be unique and ordered", entity.source
                    )
                )
