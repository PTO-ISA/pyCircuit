...
def _properties(**values: StaticValue) -> tuple[Property, ...]:
    return tuple(
        ...
        for name, value in sorted(
            values.items(), key=lambda item: utf16_sort_key(item[0])
        )
    )
...
def build_verified_acpy(
    ...
    for process in processes:
        ...
        source = (
            sites[definition.qualified_name].span if definition is not None else None
        )
        allocator.allocate(
            kind="process",
            scope=program.definition,
...
def _static_arguments(values: tuple[tuple[str, StaticValue], ...]) -> str:
    return ", ".join(f"{name} = {_static_attribute(value)}" for name, value in values)
...
def _component_declarations(program: NormalizedProgram) -> list[str]:
    ...
    for identity in sorted(schemas, key=utf16_sort_key):
        ...
        if returned:
            lines.append(
                "    ac.return " + ", ".join(returned) + " : " + ", ".join(result_types)
            )
        else:
...
def lower_to_acir(
    ...
    *,
    ...
    system = (
        f'  ac.system @{_symbol(system_name)} root @{root} as "root" tick 0 "cycle"'
    )
    ...
    names = {
        argument.name: f"%{argument.source_name}" for argument in program.arguments
    }
    for call in program.calls:
