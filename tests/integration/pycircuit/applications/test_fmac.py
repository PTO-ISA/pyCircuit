#!/usr/bin/env python3
"""
BF16 FMAC runtime regression with 100 generated test cases.
...
Build first (from pyCircuit root, with generated outputs under `.pycircuit_out`):
...
DEFAULT_LIBRARY = (
    REPOSITORY / ".pycircuit_out" / "applications" / "bf16_fmac" / "libfmac_sim.dylib"
)
...
class FmacRTL:
    def __init__(self, lib_path=None):
        ...
        library.fmac_create.restype = ctypes.c_void_p
        library.fmac_destroy.argtypes = [ctypes.c_void_p]
        library.fmac_reset.argtypes = [ctypes.c_void_p, ctypes.c_uint64]
        library.fmac_push.argtypes = [
            ctypes.c_void_p,
            ctypes.c_uint16,
            ctypes.c_uint16,
            ctypes.c_uint32,
        ]
        library.fmac_idle.argtypes = [ctypes.c_void_p, ctypes.c_uint64]
        library.fmac_get_result.argtypes = [ctypes.c_void_p]
        library.fmac_get_result.restype = ctypes.c_uint32
        library.fmac_get_result_valid.argtypes = [ctypes.c_void_p]
        library.fmac_get_result_valid.restype = ctypes.c_uint32
        library.fmac_get_cycle.argtypes = [ctypes.c_void_p]
        library.fmac_get_cycle.restype = ctypes.c_uint64
        self._L, self._c = library, library.fmac_create()
...
def main():
    ...
    print(f"\n  {'=' * 55}")
    print(
        f"  Results: {GREEN}{passed}{RESET}/{len(cases)} passed, "
