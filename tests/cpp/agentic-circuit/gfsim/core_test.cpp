...
TEST(GfsimComponentsTest, BaselineTemplatesExposeCanonicalObjectKinds) {
  Queue<uint64_t> queue("queue", 0, nullptr, 2);
  Scheduler<uint64_t> scheduler("scheduler", 1, nullptr, 2);
  Compute<> compute("compute", 2, nullptr);
  Link<> link("link", 3, nullptr);
  Memory<> memory("memory", 4, nullptr, 2);
  Sink<> sink("sink", 5, nullptr);
