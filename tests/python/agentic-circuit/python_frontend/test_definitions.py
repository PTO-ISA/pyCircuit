...
def stdlib_registry():
    ...
    return SchemaRegistry.from_catalog(
        REPOSITORY / "schemas/agentic-circuit" / "stdlib" / "catalog.json", REPOSITORY
    )
...
class DefinitionCaptureTest(unittest.TestCase):
    def test_decorated_definition_binds_without_executing_its_body(self) -> None:
        ...
        self.assertEqual(
            "Definition(kind='system', qualified_name='main')", repr(loaded.main)
        )
...
class SchemaCallableTest(unittest.TestCase):
    def test_recomputed_fingerprint_cannot_hide_invalid_closed_field(self) -> None:
        ...
        record = json.loads(
            (
                REPOSITORY / "schemas/agentic-circuit" / "stdlib" / "Queue.json"
            ).read_text()
        )
        ...
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            stdlib = root / "schemas" / "agentic-circuit" / "stdlib"
