...
def validate_schema(name: str, document: dict[str, object]) -> None:
    schema = load_json(REPOSITORY / "schemas/agentic-circuit" / name)
    ...
    def test_completed_run_publishes_exact_documents(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            executable.write_text(
                ...
                "'contract_epoch':'0.5','run_manifest':{'path':'run-manifest.json',"
            ...
            publication = execute_run(
                BuildPublication(
                    build, executable, manifest, "sha256:" + "0" * 64, False
                ),
                RunOptions(
                    trace, output, 1, None, None, (), "json", "disabled", "complete"
                ),
            )
