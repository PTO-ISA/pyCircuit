...
// RUN: %acir_cxxgen %s --stop-after=compile --output-root=%t.compile --project-name=project --project-identity=project.example --system-name=system --system-identity=system.example --profile=fast --compiler=%cxx --standard-library=libc++ --abi-mode=default --object-format=mach-o --contract-flag=-std=c++20 --include-root=%source_root/simulator/gfsim/include --include-root=%source_root/simulator/gfsim/tooling/include | %FileCheck %s --check-prefix=COMPILE
// RUN: %acir_cxxgen %s --stop-after=link --output-root=%t.link --project-name=project --project-identity=project.example --system-name=system --system-identity=system.example --profile=fast --compiler=%cxx --standard-library=libc++ --abi-mode=default --object-format=mach-o --contract-flag=-std=c++20 --include-root=%source_root/simulator/gfsim/include --include-root=%source_root/simulator/gfsim/tooling/include --link-input=%binary_root/gfsim/libgfsim_tooling.a --link-input=%binary_root/gfsim/libgfsim.a --link-input=%binary_root/lib/Bindings/libACIRBindings.a %llvm_linker_flags | %FileCheck %s --check-prefix=LINK
...
builtin.module attributes {ac.contract_epoch = "0.5"} {
  acsim.model @minimal epoch "0.5" root @Top construction [] destruction [] fingerprints {
