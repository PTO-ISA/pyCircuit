...
module attributes {ac.model_kind = "queue_graph", ac.queue_graph_domain = "cycle", ac.system = "variable_array_state"} {
  ac.type_scope @types {
    ac.struct @Entry fields [{name = "index", type = i7}, {name = "value", type = i8}]
  ...
  ac.var.decl @entries type !ac.struct<@types::@Entry> init 0 : i64 owner "/" stable_id "var/entries" shape [128]
  ...
  } {ac.query = "ready"} -> !ac.var<!ac.value_array<2 x i64>>
  %oldest, %oldest_valid = ac.var.choose @entries %ready_mask : !ac.var<!ac.value_array<2 x i64>> count 1 policy "min" key {
  ^bb0(%entry: !ac.var<!ac.struct<@types::@Entry>>):
    %index = ac.var.get %entry field "index" : !ac.var<!ac.struct<@types::@Entry>> -> !ac.var<i7>
    ac.var.choose.yield %index : !ac.var<i7>
  } {ac.query = "oldest"} -> !ac.var<i7>, !ac.var<i1>
  ...
  ^body(%item: !ac.var<!ac.struct<@types::@Entry>>):
    %index = ac.var.get %item field "index" : !ac.var<!ac.struct<@types::@Entry>> -> !ac.var<i7>
    %old = ac.var.read_element @entries[%index] : !ac.var<i7> -> !ac.var<!ac.struct<@types::@Entry>>
    ac.var.assign_element @entries[%index] = %item : !ac.var<i7>, !ac.var<!ac.struct<@types::@Entry>>
...
// STORAGE: ac.table @entries entry !ac.struct<@types::@Entry> entries 128 init 0
