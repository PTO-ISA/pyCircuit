...
builtin.module {
  ac.module @M() parameters {} attributes {ac.arch_expression_table = [{rule = "ghost", nodes = [
    ...
    ac.arch_obligation @missing id "missing" kind #ac<architecture_obligation_kind range> severity #ac<architecture_obligation_severity error> status #ac<architecture_obligation_status runtime_checked> condition {table = "ac.arch_expression_table", rule = "ghost", node = 2 : i64} rules ["ghost"] owners [{resource = @state, owner_path = "/M/state", owner_stable_id = "state"}] targets [#ac<architecture_runtime_target gfsim>] materializations [{target = #ac<architecture_runtime_target gfsim>, firing = "ghost", input_ordinal = 0 : i64, maximum = 127 : i64}] sampling {kind = #ac<architecture_sampling_kind pre_publish>, edge = #ac<architecture_sampling_edge none>, sample_anchor = "ghost", monitor_only = false} message "missing" source [{}] ndf []
    ac.return
  }
}
