...
// RUN: cd %t && env PYTHONPATH=%source_root/python/agentic-circuit/src:%binary_root/python %python -m agentic_circuit._cli compile architecture.py --emit=frozen-acir,acsim --stop-after=acsim --output-dir=output --json | %FileCheck %s
