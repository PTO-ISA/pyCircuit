...
class SW5809s:
    ...
    def __init__(self, ecmp_mode: str = "independent"):
        ...
        self.voqs = [
            [collections.deque(maxlen=VOQ_DEPTH) for _ in range(self.n_ports)]
    ...
    def voq_depth_stats(self):
        ...
        for npu in range(N_NPUS):
            ...
            port_avgs = [
                self._voq_depth_sum[p] / self._voq_snapshot_count for p in ports
            ]
            port_maxs = [self._voq_max_depth[p] for p in ports]
            npu_avg.append(sum(port_avgs) / len(port_avgs))
            npu_max.append(max(port_maxs))
        return (sum(npu_avg) / len(npu_avg), sum(npu_max) / len(npu_max), max(npu_max))
        ...
        for npu in self.npus:
            ...
            for port in range(N_NPUS):
                while sent < SW_LINKS_PER_NPU:
                    ...
                    self._to_switch.append(
                        (self.cycle + SW_LINK_LATENCY, npu.id, in_port_idx, pkt)
                    )
                    sent += 1
...
def _hist(npus, bins=12):
    lats = []
    for n in npus:
        lats.extend(n.latencies)
    if not lats:
        return [], 0, 0
    lo, hi = min(lats), max(lats)
    if lo == hi:
        return [len(lats)], lo, hi
    bw = max(1, (hi - lo + bins - 1) // bins)
    h = [0] * bins
    for latency in lats:
        h[min((latency - lo) // bw, bins - 1)] += 1
    return h, lo, hi
...
def draw(fm, sw, cycle):
    ...
    print(
        _side(
            f"{BOLD}{YELLOW}FM16 (Full Mesh){RESET}",
            f"{BOLD}{MAGENTA}SW16 (Switch){RESET}",
        )
    )
    print(
        _side(
            f"{DIM}4 links/pair, 1 hop{RESET}",
    ...
    print(
        _side(
            f"Per-NPU BW: {BOLD}{sf['per_npu_bw_gbps']:>6.0f}{RESET} Gbps",
    ...
    print(
        _side(
            f"Aggregate: {sf['agg_bw_gbps']:>8.0f} Gbps",
            f"Aggregate: {ss['agg_bw_gbps']:>8.0f} Gbps",
        )
    )
    print(_side(f"Injected:  {sf['inj']:>8d}", f"Injected:  {ss['inj']:>8d}"))
    print(_side(f"Delivered: {sf['del']:>8d}", f"Delivered: {ss['del']:>8d}"))
    ...
    print(
        _side(
            f"Avg: {YELLOW}{sf['avg']:>5.1f}{RESET}  P50:{sf['p50']:>3d}  P99:{sf['p99']:>3d}",
            f"Avg: {YELLOW}{ss['avg']:>5.1f}{RESET}  P50:{ss['p50']:>3d}  P99:{ss['p99']:>3d}",
        )
    )
    print(_side(f"Max: {sf['max_lat']:>3d} cycles", f"Max: {ss['max_lat']:>3d} cycles"))
...
def main():
    ...
    print("  Initializing 3 systems (FM16 + SW16-independent + SW16-coordinated)...")
    ...
    print(
        f"  {'Per-NPU BW (Gbps)':24s} {sf['per_npu_bw_gbps']:>14.0f} {si['per_npu_bw_gbps']:>14.0f} {sc['per_npu_bw_gbps']:>14.0f}"
    )
    print(
        f"  {'Aggregate BW (Gbps)':24s} {sf['agg_bw_gbps']:>14.0f} {si['agg_bw_gbps']:>14.0f} {sc['agg_bw_gbps']:>14.0f}"
    )
    print(
        f"  {'Avg Latency (cycles)':24s} {sf['avg']:>14.1f} {si['avg']:>14.1f} {sc['avg']:>14.1f}"
    )
    ...
    print(
        f"  {'Max Latency':24s} {sf['max_lat']:>14d} {si['max_lat']:>14d} {sc['max_lat']:>14d}"
    )
    print(
        f"  {'Delivered pkts':24s} {sf['del']:>14d} {si['del']:>14d} {sc['del']:>14d}"
    )
    print(
        f"  {'Dropped pkts':24s} {'N/A':>14s} {si.get('sw_dropped',sw_ind.switch.pkts_dropped):>14d} {sc.get('sw_dropped',sw_crd.switch.pkts_dropped):>14d}"
    )
    print(f"  {'─'*72}")

    print(f"\n  {YELLOW}ECMP VOQ Collision Analysis:{RESET}")
    print("    Each input port independently round-robins across 8 egress ports.")
    print("    'independent': 128 uncoordinated RR pointers → collisions")
    print("    'coordinated': 1 global RR per dest NPU → no collision (ideal)")
    print("")
    print(
        f"    {'Cumulative enqueue (per dest port)':40s} {'Independent':>14s} {'Coordinated':>14s}"
    )
    ...
    if li_avg > 0:
        print(
            f"    {'  Max/Avg ratio':40s} {li_max/li_avg:>14.2f}x {lc_max/lc_avg:>14.2f}x"
        )
    print("")
    print(
        f"    {'VOQ depth (per egress port)':40s} {'Independent':>14s} {'Coordinated':>14s}"
    )
    ...
    if si["p99"] > sc["p99"]:
        ...
        print("    similar latency (collision effect minimal at this load level)")
    print()
