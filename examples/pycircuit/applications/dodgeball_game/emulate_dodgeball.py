...
class DodgeballRTL:
    def __init__(self, lib_path: str | None = None):
        if lib_path is None:
            lib_path = str(
                _find_root()
                / ".pycircuit_out/applications/dodgeball_game/libdodgeball_sim.dylib"
            )
        ...
        self._lib.db_set_inputs.argtypes = [
            ctypes.c_void_p,
            ctypes.c_int,
            ctypes.c_int,
            ctypes.c_int,
    ...
    def _apply_inputs(self):
        self._lib.db_set_inputs(
            self._ctx, self.rst_btn, self.start, self.left, self.right
        )
    ...
    def ob1(self) -> tuple[int, int]:
        return (
            int(self._lib.db_get_ob1_x(self._ctx)),
            int(self._lib.db_get_ob1_y(self._ctx)),
        )

    @property
    def ob2(self) -> tuple[int, int]:
        return (
            int(self._lib.db_get_ob2_x(self._ctx)),
            int(self._lib.db_get_ob2_y(self._ctx)),
        )

    @property
    def ob3(self) -> tuple[int, int]:
        return (
            int(self._lib.db_get_ob3_x(self._ctx)),
            int(self._lib.db_get_ob3_y(self._ctx)),
        )
...
def _find_pycc(root: Path) -> Path:
    candidates = [
        root / ".pycircuit_out" / "toolchain" / "install" / "bin" / "pycc",
        root / ".pycircuit_out" / "toolchain" / "build" / "bin" / "pycc",
    ]
    for c in candidates:
    ...
    if found:
        return Path(found)
    raise RuntimeError("missing pycc (build it with: bash flows/scripts/pyc build)")


def _ensure_built(force: bool = False) -> None:
    root = _find_root()
    lib_path = (
        root / ".pycircuit_out/applications/dodgeball_game/libdodgeball_sim.dylib"
    )
    srcs = [
        root / "examples/pycircuit/applications/dodgeball_game/dodgeball_game.py",
        root / "examples/pycircuit/applications/dodgeball_game/dodgeball_capi.cpp",
    ]
    ...
    gen_dir = root / ".pycircuit_out/applications/dodgeball_game/generated"
    gen_dir.mkdir(parents=True, exist_ok=True)

    env = os.environ.copy()
    py_path = f"{root}/python/semantic-core/src:{root}/python/pycircuit/src:{root}"
    ...
    subprocess.run(
        [
            ...
            "-fPIC",
            "-I",
            str(root / ".pycircuit_out/toolchain/install/include"),
            "-I",
            str(gen_dir),
            str(gen_dir / "dodgeball_game.cpp"),
            "examples/pycircuit/applications/dodgeball_game/dodgeball_capi.cpp",
            str(root / ".pycircuit_out/toolchain/install/lib/libpyc6_runtime.a"),
            "-o",
            str(lib_path),
        ],
        cwd=root,
...
def _vga_color_at(
    ...
    *,
    ...
    sq_player = in_range(x, 40 * player_x, 40 * (player_x + 1)) and in_range(
        y, 400, 440
    )

    def sq_object(ox: int, oy: int) -> bool:
        return in_range(x, 40 * ox, 40 * (ox + 1)) and in_range(
            y, 40 * oy, 40 * (oy + 1)
        )
...
def render_vga_sampled(
    state: int, player_x: int, objects: list[tuple[int, int]]
) -> list[str]:
    lines: list[str] = []
...
def main():
    ...
    for tick in range(total_ticks):
        ...
        print(
            f"cycle={rtl.cycle}  state={state_name}  j={rtl.j}  main_clk_bit={MAIN_CLK_BIT}"
        )
        print(
            f"RST_BTN={rtl.rst_btn}  START={rtl.start}  left={rtl.left}  right={rtl.right}"
        )
