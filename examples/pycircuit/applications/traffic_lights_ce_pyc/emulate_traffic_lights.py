...
Then run:
  python examples/pycircuit/applications/traffic_lights_ce_pyc/emulate_traffic_lights.py
"""
    ...
    def __init__(self, lib_path: str | None = None):
        if lib_path is None:
            lib_path = str(
                ...
                / ".pycircuit_out/applications/traffic_lights_ce_pyc/libtraffic_lights_sim.dylib"
            )
...
def render_direction(
    label: str, tens: int, ones: int, lights: tuple[int, int, int]
) -> list[str]:
    r, y, g = lights
...
def _load_stimulus(name: str):
    ...
    try:
        return importlib.import_module(
            f"examples.pycircuit.applications.traffic_lights_ce_pyc.stimuli.{name}"
        )
