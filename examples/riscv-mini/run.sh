#!/usr/bin/env bash
# Freeze ACIR → lower to ACSim → emit C++ → compile → run the RISC-V mini core.
set -euo pipefail
...
echo
echo "exit code: ${status} (completed core reports classification=completed and diagnostic=x3=5)"
