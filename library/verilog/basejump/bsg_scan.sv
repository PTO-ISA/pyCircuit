...
`endif
   ...
   if (lo_to_hi_p) begin : reverse_bits_compat
     // Yosys does not support the SystemVerilog streaming concatenation used
     // by upstream BaseJump here.  This equivalent generate loop preserves
     // the bit-reversal semantics while keeping the vendored primitive
     // synthesizable by the runtime toolchain.
     for (genvar reverse_idx = 0; reverse_idx < width_p; reverse_idx = reverse_idx + 1)
       assign t[0][reverse_idx] = i[width_p-1-reverse_idx];
   end
   else begin : identity_bits
     assign t[0] = i;
   end
	...
	for (j = 0; j < $clog2(width_p); j = j + 1)
	  begin : row
             wire [width_p-1:0]                      fill;		  
              // Use an explicit intermediate instead of the upstream sized
              // cast (`width_p ' (...)`), which older Yosys versions parse as
              // an enormous unsized expression.
              wire [(2*width_p)-1:0] scan_concat = {fill, t[j]};
              wire [width_p-1:0] shifted = scan_concat >> (1 << j);
