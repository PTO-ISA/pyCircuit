...
// Vendored at commit 63b7c50d43e462b59506f69d341ff1e40202866d.
// Agentic Circuit portability change: function return statements are written
// as assignments so the pinned Yosys release can parse this package.
...
package cc_pkg;
  ...
  function automatic int unsigned idx_width (input int unsigned num_idx);
    if (num_idx > 32'd1) begin
      idx_width = unsigned'($clog2(num_idx));
    end else begin
      idx_width = 32'd1;
    end
  endfunction
  ...
  function automatic int unsigned ecc_get_cw_width (input int unsigned data_width);
    ...
    ecc_get_cw_width = data_width + ecc_get_parity_width(data_width);
  endfunction
